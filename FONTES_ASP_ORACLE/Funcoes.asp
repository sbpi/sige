...
Function MontaCalendario(p_mes, p_datas, p_cores, p_imagem)
   ...
   ' Define cor de fundo padrão para células de sábado e domingo
...
Sub SelecaoEscola (label, accesskey, hint, chave, chaveAux, campo, restricao, atributo)
    ...
    If chaveAux > "" Then
       SQL = SQL & "   and coalesce(sq_cliente_pai,0) = " & chaveAux & VbCrLf
    End If
...
Sub SelecaoEscolaParticular (label, accesskey, hint, chave, chaveAux, campo, restricao, atributo)
    ...
    SQL = "SELECT distinct a.sq_cliente cliente, a.sq_tipo_cliente, case b.tipo when '1' then upper(a.ds_username) else a.ds_cliente end ds_cliente, b.tipo " & VbCrLf & _
          "  from sbpi.CLIENTE            a" & VbCrLf & _
          "      inner join sbpi.Tipo_Cliente b on (a.sq_tipo_cliente = b.sq_tipo_cliente) " & VbCrLf & _
          "      inner join sbpi.Particular_Calendario c on (a.sq_cliente = c.sq_cliente) " & VbCrLf & _
...
Sub SelecaoCalendarioParticular (label, accesskey, hint, chave, chaveAux, campo, restricao, atributo)
    ...
    If Nvl(chaveAux,"nulo") <> "nulo" then          
        SQL = " SELECT sq_cliente cliente, nome from sbpi.Particular_Calendario" 
        If chaveAux > "" Then
...
Sub TrataErro(p_query)
  If instr(Err.description,"CKH_") > 0 Then ' REGISTRO TEM FILHOS
    ScriptOpen "JavaScript"
    ShowHTML " alert('Não é permitido usar palavras ""script"" ou "".js"" em nenhum campo.');"
    Err.clear
...
SQL = "select homologado, sq_particular_calendario, nome from sbpi.Particular_Calendario " & VbCrLf & _
