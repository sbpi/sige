...
Sub Inicial
  ...
  If O = "L" Then
     SQL = "select a.sq_cliente_mail, a.enviada, a.assunto, substr(a.texto,1,200) texto, " & VbCrLf & _
           "       to_char(a.data_inclusao, 'dd/mm/yyyy') data_inclusao, a.data_inclusao dt_inclusao," & VbCrLf & _
           "       case when a.data_envio is null then '---' else to_char(a.data_envio, 'dd/mm/yyyy') end data_envio, " & VbCrLf & _
           "       a.envia_lista, case a.envia_lista when 'S' then 'Sim' else 'Não' end nm_envia_lista, " & VbCrLf & _
           "       (select count(*) Itens from sbpi.Mail_Destinatario where sq_cliente_mail = a.sq_cliente_mail) Itens " & VbCrLf & _
           "from sbpi.Cliente_Mail        a " & VbCrLf & _
           "     inner join sbpi.Cliente  b on (a.sq_cliente = b.sq_cliente) " & VbCrLf
     ...
     SQL = SQL & "order by a.data_inclusao desc"
     ConectaBD SQL
  ...
  ElseIf Instr("IAEV",O) > 0 Then
    ...
    If O <> "I" Then
       ...
       SQL = "select sq_mail_anexo, arquivo_origem, arquivo_destino " & VbCrLf & _
             "  from sbpi.Cliente_Mail          a " & VbCrLf & _
             "       inner join sbpi.Mail_Anexo b on (a.sq_cliente_mail        = b.sq_cliente_mail) " & VbCrLf & _
       ...
       SQL = "select c.sq_cliente, c.ds_cliente nome, a.envia_lista " & VbCrLf & _
             "  from sbpi.Cliente_Mail                   a " & VbCrLf & _
             "       left outer join sbpi.Mail_Destinatario b on (a.sq_cliente_mail  = b.sq_cliente_mail) " & VbCrLf & _
             "         inner join sbpi.Cliente           c on (b.destinatario     = c.sq_cliente) " & VbCrLf & _
             " where a.sq_cliente_mail = " & w_sq_cliente_mail & " " & VbCrLf & _
             "order by 2 "
       Set RS1 = dbms.Execute(SQL)
       w_Cont = 0
  ...
  If InStr("L",O) Then
     If SG = "DESTINO" Then ' Se for destinatário
        SQL = "select c.sq_cliente, c.ds_cliente nome " & VbCrLf & _
              "  from sbpi.Cliente_Mail                   a " & VbCrLf & _
              "       inner   join sbpi.Mail_Destinatario b on (a.sq_cliente_mail = b.sq_cliente_mail) " & VbCrLf & _
              "         inner join sbpi.Cliente           c on (b.destinatario    = c.sq_cliente)" & VbCrLf & _
              " where a.sq_cliente_mail = " & w_sq_cliente_mail & " " & VbCrLf & _
              "order by 2 "
    ...
    If SG = "DESTINO" Then ' Se for destinatário da mensagem
       ...
       If Request("pesquisa") > "" Then
          sql = "SELECT DISTINCT d.sq_cliente, d.ds_cliente, d.ds_apelido, " & VbCrLf & _
                "       coalesce(d.ds_email, 'não informado') ds_email, " & VbCrLf & _ 
                "       coalesce(e.ds_email_contato, 'não informado') ds_email_contato, " & VbCrLf & _ 
                "       coalesce(f.ds_email_internet, 'não informado') ds_email_internet " & VbCrLf & _ 
                "  from sbpi.Cliente                                 d " & VbCrLf & _ 
                "       LEFT OUTER join sbpi.Especialidade_cliente   c ON (c.sq_cliente       = d.sq_cliente) " & VbCrLf & _
                "       LEFT OUTER join sbpi.Especialidade           a ON (a.sq_especialidade = c.sq_especialidade) " & VbCrLf & _
                "       LEFT OUTER join sbpi.Cliente_Dados           e ON (d.sq_cliente       = e.sq_cliente) " & VbCrLf & _
                "       LEFT OUTER join sbpi.Cliente_Site            f ON (d.sq_cliente       = f.sq_cliente) " & VbCrLf & _
                ...
                "                          from sbpi.Mail_Destinatario " & VbCrLf & _
                ...
                "   where coalesce(h.existe,0) = 0 " & VbCrLf
...
Public Sub Grava
  ...
  If O = "I" Then
     ...
     If SG = "DESTINO" Then ' Se for destinatário
        For i = 1 To Request.Form("w_sq_cliente").Count
           ...
           If Rs("Existe") = 0 Then
              SQL = "insert into sbpi.Mail_Destinatario " & VbCrLf & _
     ...
     ElseIf SG = "ANEXO" Then ' Se for arquivo atachado
        ...
        If ul.Files("p_regional").OriginalPath > "" Then
           ...
           ' Grava o registro
           SQL = "insert into sbpi.Mail_Anexo " & VbCrLf & _
     ...
     Else
        ...
        SQL = "select sbpi.sq_cliente_mail.nextval chave from sbpi.Cliente_Mail"
        ConectaBD SQL
        w_mensagem = RS("chave")
        DesconectaBD
        
        SQL = "insert into sbpi.Cliente_Mail " & VbCrLf & _
  ...
  ElseIf O = "A" Then
     dbms.BeginTrans()
     SQL = "update sbpi.Cliente_Mail set " & VbCrLf & _
  ...
  ElseIf O = "E" Then
     ...
     Else
        ...
        SQL = "delete sbpi.Mail_Anexo where sq_cliente_mail = " & Request("w_sq_cliente_mail")
        ExecutaSQL(SQL)
        ' Apaga os destinatários da mensagem
        SQL = "delete sbpi.Mail_Destinatario where sq_cliente_mail = " & Request("w_sq_cliente_mail")
        ExecutaSQL(SQL)
        ' Apaga a mensagem
        SQL = "delete sbpi.Cliente_Mail where sq_cliente_mail = " & Request("w_sq_cliente_mail")
     End If
  ElseIf O = "V" Then
     ' Define variáveis necessárias ao envio da mensagem
     Dim Mail
     ...
     SQL = "select a.sq_cliente_mail, a.envia_lista, b.ds_cliente nome, a.assunto, a.texto, b.ds_email email, b.ds_username " & VbCrLf & _
           "from sbpi.Cliente_Mail              a  " & VbCrLf & _
           "     inner   join sbpi.Cliente      b  on (a.sq_cliente_remetente = b.sq_cliente) " & VbCrLf & _
     ...
     SQL = "select a.arquivo_origem, a.arquivo_destino " & VbCrLf & _
           "from sbpi.Mail_Anexo a  " & VbCrLf & _
           "where sq_cliente_mail = " & Request("w_sq_cliente_mail")
     ConectaBD SQL
     If Not RS.EOF Then
     ...
     If w_lista = "S" Then
        ...
        SQL = "select * from sbpi.NewsLetter a where envia_mail = 'S'"
        ConectaBD SQL
        If Not RS.EOF Then
          While Not RS.EOF
     ...
     SQL = "select b.ds_cliente nome, b.ds_email email " & VbCrLf & _
           "from sbpi.Mail_Destinatario  a  " & VbCrLf & _
           "     inner join sbpi.Cliente b on (a.destinatario = b.sq_cliente) " & VbCrLf & _
           "where a.sq_cliente_mail = " & Request("w_sq_cliente_mail")
     ConectaBD SQL
     If Not RS.EOF Then
       While Not RS.EOF
     ...
     dbms.BeginTrans()
     ' Atualiza a mensagem
     SQL = "update sbpi.Cliente_Mail set data_envio = sysdate where sq_cliente_mail = " & Request("w_sq_cliente_mail")
