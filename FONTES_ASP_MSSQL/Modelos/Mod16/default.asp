...
<!--ShowHTML "  <script language=""javascript"" src=""collapse.js""> </script>"-->
...
Public Sub ShowArquivoCorreto
  ...
  sql = "SELECT case a.pasta when '1' then (a.pasta*1000000)+(month(a.dt_arquivo)*10000)+a.nr_ordem else (a.pasta*1000000)+a.nr_ordem end as ordena, case in_destinatario when 'A' then 'Aluno' when 'P' then 'Professor' when 'E' then 'Escola' else 'Todos' end AS in_destinatario, " & VbCrLf & _
    "    dt_arquivo, ds_titulo, nr_ordem, pasta, case a.pasta when '1' then 'Meses' when '2' then 'Formulários' when '3' then 'Diversos' else '' end as nmpasta, ds_arquivo, ln_arquivo, 'Escola' AS Origem, x.ln_internet diretorio " & VbCrLf & _
    ...
    "UNION " & VbCrLf & _
    "SELECT case a.pasta when '1' then (a.pasta*1000000)+(month(a.dt_arquivo)*10000)+a.nr_ordem else (a.pasta*1000000)+a.nr_ordem end as ordena, case a.in_destinatario when 'A' then 'Aluno' when 'P' then 'Professor' when 'E' then 'Escola' else 'Todos' end AS in_destinatario, " & VbCrLf & _
  ...
  ShowHTML "<tr><td><TABLE id=""tbarquivos"" border=0 cellSpacing=5 width=""95%"">"
  wCont = 0
  If Not RS.EOF Then
  wCont = 1
  Do While Not RS.EOF
   For i = 1 to 3 step 1
    If(RS("pasta") <> "" AND cInt(RS("pasta")) = i AND pasta <> i) Then
     ShowHTML "  <TR>"
     ShowHTML "  <TD><H5><img style=""vertical-align:middle border:none;"" src=""img/pasta.gif""/>&nbsp;"&RS("nmpasta")&"</H5></TD>"
     ShowHTML "  </TR>"
     If(cInt(RS("pasta")) <> 1)Then
      ShowHTML "  <TR id=""demaispastas"">"
    ...
    for j = 1 to 12 step 1
      If(cInt(RS("pasta")) = i AND cInt(Month(RS("dt_arquivo"))) = j AND mes <> j)Then
        If(mes <> j AND cInt(RS("pasta")) = 1)Then
          ShowHTML "  <TR>"
          ShowHTML "  <TD BGCOLOR=""#FAFAFA"" COLSPAN=4><H6><img style=""vertical-align:middle border:none;"" src=""img/pasta.gif""/>&nbsp;"&exibeMes(RS("dt_arquivo"))&"/"&year(RS("dt_arquivo"))&"</H6></TD>"
          ShowHTML "  </TR>"
          ShowHTML "  <TR class=""pastameses"">"
          ShowHTML "  <TD><b>Origem"
          ShowHTML "  <TD><b>Alvo"
          ShowHTML "  <TD><b>Data"
          ShowHTML "  <TD><b>Componente curricular"
     ...
     If(cInt(RS("pasta")) = i) Then
      pasta = cInt(RS("pasta"))
      If(Month(RS("dt_arquivo"))=j) Then
       If(cInt(RS("pasta")) = 1)Then
        ShowHTML "  <TR class=""pastameses"" valign=""top"">"
       Else
        ShowHTML "  <TR id=""demaispastas"" valign=""top"">"
       End If
  ...
  RS.Open sql, sobjConn, adOpenForwardOnly
  If Not RS.EOF Then
    ShowHTML "  <TR><TD colspan=5><b>Arquivos de outros anos</b><br>"
    While Not RS.EOF
...
Public Sub ShowArquivo
  ...
  sql = "SELECT case a.pasta when '1' then (a.pasta*1000000)+(12-month(a.dt_arquivo)*10000)+a.nr_ordem else (a.pasta*1000000)+(12-month(a.dt_arquivo)*10000)+a.nr_ordem end as ordena, case in_destinatario when 'A' then 'Aluno' when 'P' then 'Professor' when 'E' then 'Escola' else 'Todos' end AS in_destinatario, " & VbCrLf & _
    "    dt_arquivo, ds_titulo, nr_ordem, pasta, case a.pasta when '1' then 'Meses' when '2' then 'Formulários' when '3' then 'Diversos' else '' end as nmpasta, ds_arquivo, ln_arquivo, 'Escola' AS Origem, x.ln_internet diretorio " & VbCrLf & _
    "From escCliente_Arquivo a, escCliente x " & VbCrLf & _
    "WHERE in_ativo = 'Sim'" & VbCrLf & _
    "  AND x." & sstrEF & " " & VbCrLf & _
    ...
    "  and YEAR(a.dt_arquivo) = " & wAno & VbCrLf & _
    "UNION " & VbCrLf & _
    "SELECT case a.pasta when '1' then (a.pasta*10000)+(12-month(a.dt_arquivo)*10000)+a.nr_ordem else (a.pasta*1000000)+(12-month(a.dt_arquivo)*10000)+a.nr_ordem end as ordena, case a.in_destinatario when 'A' then 'Aluno' when 'P' then 'Professor' when 'E' then 'Escola' else 'Todos' end AS in_destinatario, " & VbCrLf & _
    "    a.dt_arquivo, a.ds_titulo, a.nr_ordem, a.pasta, case a.pasta when '1' then 'Meses' when '2' then 'Formulários' when '3' then 'Diversos' else '' end as nmpasta, ds_arquivo, ln_arquivo, 'SEDF' AS Origem, d.ds_diretorio diretorio " & VbCrLf & _
    "From escCliente_Arquivo AS a INNER JOIN escCliente AS b ON (a.sq_site_cliente = b.sq_cliente) " & VbCrLf & _
    "              INNER JOIN escCliente AS c ON (c.sq_cliente_pai  = b.sq_cliente) " & VbCrLf & _
    "              INNER JOIN escCliente AS e ON (e.sq_cliente_pai  = c.sq_cliente) " & VbCrLf & _
  ...
  If Not RS.EOF Then
    ShowHTML "  </UL>"
    ShowHTML "  <TR><TD colspan=5><br><b>Arquivos de outros anos</b><br>"
    While Not RS.EOF
     ShowHTML "   <li><a href=""" & w_dir & "default.asp?EW=143&CL=" & replace(CL,"sq_cliente=","") & "&wAno=" & RS("ano") & """ >Arquivos de " & RS("ano") & "</a></li>"
     RS.MoveNext
    Wend
...
Public Sub ShowCalend
  ...
  sql = "SELECT '#99CCFF' as cor, b.imagem, a.dt_ocorrencia AS data, 'IE: ' + a.ds_ocorrencia AS ocorrencia, 'E' AS origem FROM escCalendario_Cliente a inner join escTipo_Data b on (a.sq_tipo_data = b.sq_tipo_data and b.sigla not in ('RE','RA')) WHERE sq_site_cliente = " & CL & "  AND YEAR(dt_ocorrencia)= " & Year(Date()) & " " & VbCrLf & _
     "ORDER BY data, origem desc, ocorrencia" & VbCrLf
...
Public Sub ShowSenha
  ...
  If sstrIN = 0 Then
    ...
    ShowHTML "  window.open('/"& w_dir &"Aluno.asp?EW=118&EF=" & sstrEF & "&EA=sq_aluno=" & w_chave & "&CL=" & CL & "&IN=10" & "', 'aluno', 'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=780,height=500,left=10,top=10');" & VbCrLf
