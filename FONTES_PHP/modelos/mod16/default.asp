<%@ LANGUAGE="VBSCRIPT" %>
<!--#INCLUDE VIRTUAL="/sige_ora/Constants_ADO.inc"-->
<!--#INCLUDE VIRTUAL="/sige_ora/modelos/Constants.inc"-->
<!--#INCLUDE VIRTUAL="/sige_ora/esc.inc"-->
<!--#INCLUDE VIRTUAL="/sige_ora/Funcoes.asp"-->
<%
...
ShowHTML "   <link href=""/sige_ora/css/estilo.css"" media=""screen"" rel=""stylesheet"" type=""text/css"" />"
ShowHTML "   <link href=""/sige_ora/css/print.css""  media=""print""  rel=""stylesheet"" type=""text/css"" />"
...
sql = "SELECT a.sq_cliente, a.ds_cliente, b.ds_mensagem from sbpi.Cliente a inner join sbpi.cliente_site b on (a.sq_cliente = b.sq_cliente)WHERE a." & sstrEF
...
Public Sub ShowArquivo
  ...
  sql = "SELECT case in_destinatario when 'A' then 'Aluno' when 'P' then 'Professor' when 'E' then 'Escola' else 'Todos' end in_destinatario, " & VbCrLf & _
        "       dt_arquivo, ds_titulo, nr_ordem, ds_arquivo, ln_arquivo, 'Escola' Origem, x.ln_internet diretorio " & VbCrLf & _
        "from sbpi.Cliente_Arquivo a, sbpi.cliente x " & VbCrLf & _
        "WHERE a.ativo = 'S'" & VbCrLf & _
        "  AND x." & sstrEF & " " & VbCrLf & _
        "  AND a." & replace(sstrEF,"sq_cliente","sq_cliente") & " " & VbCrLf & _
        "  and in_destinatario <> 'E'" & VbCrLf & _
        "  and sbpi.year(a.dt_arquivo) = " & wAno & VbCrLf & _
        "UNION " & VbCrLf & _
        "SELECT case a.in_destinatario when 'A' then 'Aluno' when 'P' then 'Professor' when 'E' then 'Escola' else 'Todos' end in_destinatario, " & VbCrLf & _
        "       a.dt_arquivo, a.ds_titulo, a.nr_ordem, ds_arquivo, ln_arquivo, 'SEDF' Origem, d.ds_diretorio diretorio " & VbCrLf & _
        "from sbpi.Cliente_Arquivo a INNER join sbpi.Cliente b ON (a.sq_cliente = b.sq_cliente) " & VbCrLf & _
        "                             INNER join sbpi.Cliente c ON (c.sq_cliente_pai  = b.sq_cliente) " & VbCrLf & _
        "                             INNER join sbpi.Cliente e ON (e.sq_cliente_pai  = c.sq_cliente) " & VbCrLf & _
        "                             INNER join sbpi.Cliente_Site d ON (b.sq_cliente = d.sq_cliente) " & VbCrLf & _
        "WHERE a.ativo = 'S'" & VbCrLf & _
        ...
        "UNION " & VbCrLf & _
        "SELECT case a.in_destinatario when 'A' then 'Aluno' when 'P' then 'Professor' when 'E' then 'Escola' else 'Todos' end in_destinatario, " & VbCrLf & _
        "       a.dt_arquivo, a.ds_titulo, a.nr_ordem, ds_arquivo, ln_arquivo, 'Regional' Origem, d.ds_diretorio diretorio " & VbCrLf & _
        "from sbpi.Cliente_Arquivo a INNER join sbpi.Cliente c ON (a.sq_cliente = c.sq_cliente) " & VbCrLf & _
        "                             INNER join sbpi.Cliente e ON (e.sq_cliente_pai  = c.sq_cliente) " & VbCrLf & _
        "                             INNER join sbpi.Cliente_Site d ON (c.sq_cliente = d.sq_cliente) " & VbCrLf & _
        "WHERE a.ativo = 'S'" & VbCrLf & _
        "  AND e." & sstrEF & " " & VbCrLf & _
        "  and in_destinatario <> 'E'" & VbCrLf & _
        "  and sbpi.year(a.dt_arquivo) = " & wAno & VbCrLf & _
  ...
  sql = "SELECT sbpi.year(dt_arquivo) ano " & VbCrLf & _
        ...
        "  and sbpi.year(a.dt_arquivo) <> " & wAno & VbCrLf & _
        "UNION " & VbCrLf & _
        "SELECT sbpi.year(dt_arquivo) ano " & VbCrLf & _
        "from sbpi.Cliente_Arquivo a INNER join sbpi.Cliente b ON (a.sq_cliente = b.sq_cliente) " & VbCrLf & _
        "                             INNER join sbpi.Cliente c ON (c.sq_cliente_pai  = b.sq_cliente) " & VbCrLf & _
        "                             INNER join sbpi.Cliente e ON (e.sq_cliente_pai  = c.sq_cliente) " & VbCrLf & _
        "                             INNER join sbpi.Cliente_Site d ON (b.sq_cliente = d.sq_cliente) " & VbCrLf & _
...
Public Sub ShowPrincipal
  ...
  sql = "SELECT b.im_logo, b.im_foto_abertura1, b.im_foto_abertura2, b.ds_diretorio, b.ds_texto_abertura, e.sq_cliente_foto, e.ln_foto, e.ds_foto " & _
        "from sbpi.Cliente a INNER join sbpi.CLIENTE_SITE  b on (a.sq_cliente = b.sq_cliente) " & _
        "                     INNER join sbpi.Modelo        c on (b.sq_modelo = c.sq_modelo) " & _
        "                     INNER join sbpi.Cliente_Dados d on (a.sq_cliente = d.sq_cliente) " & _
        "                     LEFT  join sbpi.Cliente_Foto  e on (a.sq_cliente = e.sq_cliente and e.tp_foto = 'P') " & _
...
Public Sub ShowQuem
  ...
  itemmenu = "Quem Somos"
   ...
   sql = "SELECT b.ds_institucional, b.ds_diretorio, e.sq_cliente_foto, e.ln_foto, e.ds_foto " & _
        "from sbpi.Cliente a INNER join sbpi.CLIENTE_SITE  b on (a.sq_cliente = b.sq_cliente) " & _
        "                     INNER join sbpi.Modelo        c on (b.sq_modelo = c.sq_modelo) " & _
        "                     LEFT  join sbpi.Cliente_Dados d on (a.sq_cliente = d.sq_cliente) " & _
        "                     LEFT  join sbpi.Cliente_Foto  e on (a.sq_cliente = e.sq_cliente and e.tp_foto = 'Q') " & _
        "WHERE a." & sstrEF & " " & VbCrLf & _
        "ORDER BY e.nr_ordem"
...
Public Sub ShowExFale
  ...
  sql = "SELECT c.no_diretor, c.no_secretario, c.ds_logradouro, c.no_bairro, c.nr_cep, b.no_contato_internet, b.ds_email_internet, b.nr_fone_internet, b.nr_fax_internet, a.no_municipio, a.sg_uf " & _
        "from sbpi.Cliente a INNER join sbpi.CLIENTE_SITE b on (a.sq_cliente = b.sq_cliente) " & _
        "                     INNER join sbpi.CLIENTE_Dados c on (a.sq_cliente = c.sq_cliente) " & _
        "WHERE a." & sstrEF 
...
Public Sub ShowNoticia
  ...
  If sstrIN = 0 then
     sql = "SELECT a.sq_noticia, a.dt_noticia data, a.ds_titulo ocorrencia, 'Escola' origem " & VbCrLf & _
           "  from sbpi.Noticia_Cliente  a" & VbCrLf & _
           " WHERE sq_cliente   = " & CL & VbCrLf & _
           "  and a.ativo         = 'S'" & VbCrLf & _
           "  and a.in_exibe         = 'S'" & VbCrLf & _
           "  and sbpi.year(a.dt_noticia) = " & wAno & VbCrLf & _
           "UNION " & VbCrLf & _
           "SELECT a.sq_noticia, dt_noticia data, ds_titulo ocorrencia, 'Regional' origem " & VbCrLf & _
           "from sbpi.Noticia_Cliente    a" & VbCrLf & _
           "     INNER join sbpi.Cliente b ON (a.sq_cliente = b.sq_cliente) " & VbCrLf & _
           "     INNER join sbpi.Cliente c ON (b.sq_cliente      = c.sq_cliente_pai) " & VbCrLf & _
           "WHERE c.sq_cliente       = " & CL & VbCrLf & _
           "  and a.ativo         = 'S'" & VbCrLf & _
           "  and a.in_exibe         = 'S'" & VbCrLf & _
           "  and sbpi.year(a.dt_noticia) = " & wAno & VbCrLf & _
           "UNION " & VbCrLf & _
           "SELECT a.sq_noticia, dt_noticia data, ds_titulo ocorrencia, 'SEDF' origem " & VbCrLf & _
           "from sbpi.Noticia_Cliente    a" & VbCrLf & _
           "     INNER join sbpi.Cliente b ON (a.sq_cliente = b.sq_cliente) " & VbCrLf & _
           "     INNER join sbpi.Cliente c ON (b.sq_cliente      = c.sq_cliente_pai) " & VbCrLf & _
           "     INNER join sbpi.Cliente d ON (c.sq_cliente      = d.sq_cliente_pai) " & VbCrLf & _
     ...
     End If
     RS.Close
     sql = "SELECT sbpi.year(a.dt_noticia) ano " & VbCrLf & _
           "  from sbpi.Noticia_Cliente  a" & VbCrLf & _
           " WHERE sq_cliente   = " & CL & VbCrLf & _
           "  and a.ativo         = 'S'" & VbCrLf & _
           "  and a.in_exibe         = 'S'" & VbCrLf & _
           "  and sbpi.year(a.dt_noticia) <> " & wAno & VbCrLf & _
           "UNION " & VbCrLf & _
           "SELECT sbpi.year(a.dt_noticia) ano " & VbCrLf & _
           "from sbpi.Noticia_Cliente    a" & VbCrLf & _
           "     INNER join sbpi.Cliente b ON (a.sq_cliente = b.sq_cliente) " & VbCrLf & _
           "     INNER join sbpi.Cliente c ON (b.sq_cliente      = c.sq_cliente_pai) " & VbCrLf & _
           "WHERE c.sq_cliente       = " & CL & VbCrLf & _
...
Public Sub ShowCalend
  ...
  sql = "SELECT '' cor, b.imagem, a.dt_ocorrencia data, a.ds_ocorrencia ocorrencia, 'B' origem from sbpi.Calendario_base a left join sbpi.Tipo_Data b on (a.sq_tipo_data = b.sq_tipo_data) WHERE sbpi.year(dt_ocorrencia)=" & year(Date()) & " " & VbCrLf & _
        "UNION " & VbCrLf & _
        "SELECT '#99CCFF' cor, b.imagem, a.dt_ocorrencia data, a.ds_ocorrencia ocorrencia, 'E' origem from sbpi.Calendario_Cliente a left join sbpi.Tipo_Data b on (a.sq_tipo_data = b.sq_tipo_data) WHERE sq_cliente = " & CL & "  AND sbpi.year(dt_ocorrencia)= " & year(Date()) & " " & VbCrLf & _
        "UNION " & VbCrLf & _
        "SELECT '#FFFF99' cor, e.imagem, a.dt_ocorrencia data, a.ds_ocorrencia ocorrencia, 'R' origem " & VbCrLf & _
        "from sbpi.Calendario_Cliente   a" & VbCrLf & _
        "     INNER join sbpi.Cliente   b ON (a.sq_cliente = b.sq_cliente) " & VbCrLf & _
        "     INNER join sbpi.Cliente   c ON (b.sq_cliente      = c.sq_cliente_pai) " & VbCrLf & _
        "     INNER join sbpi.Cliente   d ON (c.sq_cliente      = d.sq_cliente_pai) " & VbCrLf & _
        "     LEFT  join sbpi.Tipo_Data e ON (a.sq_tipo_data    = e.sq_tipo_data) " & VbCrLf & _
        "WHERE d.sq_cliente = " & CL & VbCrLf & _
        "  AND sbpi.year(dt_ocorrencia)= " & wAno & " " & VbCrLf & _
  ...
  If sstrIN = 0 then
     ...
     ShowHTML "<tr valign=""top"">"
     'Legenda
     sql = "SELECT * from sbpi.Tipo_Data where abrangencia <> 'U' order by nome " & VbCrLf
     RS.Open sql, sobjConn, adOpenForwardOnly
     ...
     If Not RS.EOF Then
        ...
        Do While Not RS.EOF
           ShowHTML "  <TR VALIGN=""TOP"">"
           ShowHTML "    <TD>&nbsp;"
           ShowHTML "    <TD><img src=""/sige_ora/img/" & RS("imagem") & """ align=""center"">"
           ShowHTML "    <TD>" & RS("nome")
           ShowHTML "  </TR>"
           RS.MoveNext
        Loop
     ...
     RS.Close
     'Feriados
     sql = "SELECT '' cor, b.imagem, a.dt_ocorrencia data, a.ds_ocorrencia ocorrencia, 'B' origem from sbpi.Calendario_base a left join sbpi.Tipo_Data b on (a.sq_tipo_data = b.sq_tipo_data) WHERE sbpi.year(dt_ocorrencia)=" & wAno & " AND b.sigla <> 'CN' " & VbCrLf & _
           "ORDER BY data, origem desc, ocorrencia" & VbCrLf
     RS.Open sql, sobjConn, adOpenForwardOnly
     If Not RS.EOF Then
        ShowHTML "  <td><TABLE width=""90%"" align=""center"" border=0 cellSpacing=1>"
     ...
     ' Exibe recessos
     sql = "SELECT '#99CCFF' cor, b.imagem, a.dt_ocorrencia data, a.ds_ocorrencia ocorrencia, 'E' origem from sbpi.Calendario_Cliente a inner join sbpi.Tipo_Data b on (a.sq_tipo_data = b.sq_tipo_data and b.sigla in ('RE','RA')) WHERE sq_cliente = " & CL & "  AND sbpi.year(dt_ocorrencia)= " & year(Date()) & " " & VbCrLf & _
           "UNION " & VbCrLf & _
           "SELECT '#FFFF99' cor, e.imagem, a.dt_ocorrencia data, a.ds_ocorrencia ocorrencia, 'R' origem " & VbCrLf & _
           "from sbpi.Calendario_Cliente   a" & VbCrLf & _
           "     INNER join sbpi.Cliente   b ON (a.sq_cliente = b.sq_cliente) " & VbCrLf & _
           "     INNER join sbpi.Cliente   c ON (b.sq_cliente      = c.sq_cliente_pai) " & VbCrLf & _
           "     INNER join sbpi.Cliente   d ON (c.sq_cliente      = d.sq_cliente_pai) " & VbCrLf & _
           "     INNER  join sbpi.Tipo_Data e ON (a.sq_tipo_data    = e.sq_tipo_data and e.sigla in ('RE','RA')) " & VbCrLf & _
           "WHERE d.sq_cliente = " & CL & VbCrLf & _
           "  AND sbpi.year(dt_ocorrencia)= " & wAno & " " & VbCrLf & _
           "ORDER BY data, origem desc, ocorrencia" & VbCrLf
     ...
     sql = "SELECT '#99CCFF' cor, b.imagem, a.dt_ocorrencia data, 'IE: ' || a.ds_ocorrencia ocorrencia, 'E' origem from sbpi.Calendario_Cliente a inner join sbpi.Tipo_Data b on (a.sq_tipo_data = b.sq_tipo_data and b.sigla not in ('RE','RA')) WHERE sq_cliente = " & CL & "  AND sbpi.year(dt_ocorrencia)= " & year(Date()) & " " & VbCrLf & _
           "ORDER BY data, origem desc, ocorrencia" & VbCrLf
     RS.Open sql, sobjConn, adOpenForwardOnly
     ...
     sql = "  select * from " & VbCrLf & _
           ...
           "           from sbpi.Calendario_Cliente a inner join sbpi.Tipo_Data b on (a.sq_tipo_data = b.sq_tipo_data and sbpi.year(a.dt_ocorrencia) = " &  wAno &  "and b.sigla = 'IA') " & VbCrLf & _
           "          where sq_cliente = " & 0 & VbCrLf & _
           "        ) a, " & VbCrLf & _
           "        (select dt_ocorrencia w_let_fim " & VbCrLf & _
           "           from sbpi.Calendario_Cliente a inner join sbpi.Tipo_Data b on (a.sq_tipo_data = b.sq_tipo_data and sbpi.year(a.dt_ocorrencia) = " &  wAno &  " and b.sigla = 'TA') " & VbCrLf & _
           "          where sq_cliente = " & 0 & VbCrLf & _
           "        ) b, " & VbCrLf & _
           "        (select dt_ocorrencia w_let2_ini " & VbCrLf & _
           "           from sbpi.Calendario_Cliente a inner join sbpi.Tipo_Data b on (a.sq_tipo_data = b.sq_tipo_data and sbpi.year(a.dt_ocorrencia) = " &  wAno &  " and b.sigla = 'I2') " & VbCrLf & _
           "          where sq_cliente = " & 0 & VbCrLf & _
           "        ) c, " & VbCrLf & _
           "        (select dt_ocorrencia w_let1_fim " & VbCrLf & _
           "           from sbpi.Calendario_Cliente a inner join sbpi.Tipo_Data b on (a.sq_tipo_data = b.sq_tipo_data and sbpi.year(a.dt_ocorrencia) = " &  wAno &  " and b.sigla = 'T1') " & VbCrLf & _
           "          where sq_cliente = " & 0 & VbCrLf & _
           "        ) d " & VbCrLf
     RS.Open sql, sobjConn, adOpenForwardOnly
     Do While Not RS.EOF
     ...
     For w_cont = 1 to 2
        ...
        For wCont = w_ini to w_fim
           ...
           sql = "SELECT sbpi.diasLetivos('" & w_inicial & "', '" & w_final & "'," & CL & ",null) qtd from dual" & VbCrLf 
           RS.Open sql, sobjConn, adOpenForwardOnly
...
%><!--
     ...
     ShowHTML "  </TR>"
     For wCont = 1 to 12
        sql = "SELECT sbpi.diasLetivos('01/" & mid(100+wCont,2,2) & "/'||cast(sbpi.year(sysdate) as varchar),to_char(sbpi.lastDay(to_char,'01/" & mid(100+wCont,2,2) & "/'||cast(sbpi.year(sysdate) as varchar),'dd/mm/yyyy')),'dd/mm/yyyy')," & CL & ") qtd" & VbCrLf 
        RS1.Open sql, sobjConn, adOpenForwardOnly
...
Public Sub ShowSenha
  ...
  If sstrIN = 0 Then
     sql = "SELECT * from sbpi.Aluno " & VbCrLf & _
           "WHERE sq_cliente = " & CL & VbCrLf & _
           "  AND NR_MATRICULA   ='" & w_uid & "'" & VbCrLf & _
           "  AND DS_SENHA_ACESSO='" & w_pwd & "'" & VbCrLf
  Else
     sql = "SELECT * from sbpi.Cliente " & VbCrLf & _
           "WHERE sq_cliente     = " & CL & VbCrLf & _
...
Dim sql2, biblioteca, linguas
    ...
    "   and 0 < (select count(sq_cliente) from sbpi.Particular_curso where sq_cliente = " & CL & ") " & VbCrLf & _
    "ORDER BY ds_especialidade "
    RS1.Open sql2, sobjConn, adOpenForwardOnly
    w_cont = 0
