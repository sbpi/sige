  ...
  function ExibeDocs() {
    ...
    if ($SG == '' || ($SG > '' && ($O == 'L' || $O == 'P'))) {
      ...
      foreach ($RS as $row) {
        ...
        if (f($row, 'externo') == 'S')
          eval ('$node1_' .
          i . ' = &$node1->addItem(new XNode(null,LinkArquivo(\'hl\',$p_cliente,str_replace(\'@files/\',\'\',f($row,\'LINK\')),f($row,\'target\'),null,\'<img src="\'.$w_Imagem.\'" border=0>\'.f($row,\'nome\'),null),null,null,null));');
        else {
          if ($_REQUEST['w_cgccpf'] > '')
            eval ('$node' .
            i . ' = &$node1->addItem(new XNode(f($row,\'nome\'),f($row,\'LINK\').\'&P1=\'.f($row,\'P1\').\'&P2=\'.f($row,\'P2\').\'&P3=\'.f($row,\'P3\').\'&P4=\'.f($row,\'P4\').\'&TP=\'.$w_titulo.\'&SG=\'.f($row,\'SIGLA\').\'&O=L&w_cgccpf=\'.$_REQUEST[\'w_cgccpf\'].MontaFiltro(\'GET\'),$w_Imagem,$w_Imagem,f($row,\'target\')));');
          elseif ($_REQUEST['w_usuario'] > '') eval ('$node' .
          i . ' = &$node1->addItem(new XNode(f($row,\'nome\'),f($row,\'LINK\').\'&P1=\'.f($row,\'P1\').\'&P2=\'.f($row,\'P2\').\'&P3=\'.f($row,\'P3\').\'&P4=\'.f($row,\'P4\').\'&TP=\'.$w_titulo.\'&SG=\'.f($row,\'SIGLA\').\'&O=L&w_usuario=\'.$_REQUEST[\'w_usuario\'].\'&w_menu=\'.f($row,\'menu_pai\'),$w_Imagem,$w_Imagem,f($row,\'target\')));');
          elseif ($_REQUEST['w_sq_acordo'] > '') eval ('$node' .
          i . ' = &$node1->addItem(new XNode(f($row,\'nome\'),f($row,\'LINK\').\'&P1=\'.f($row,\'P1\').\'&P2=\'.f($row,\'P2\').\'&P3=\'.f($row,\'P3\').\'&P4=\'.f($row,\'P4\').\'&TP=\'.$w_titulo.\'&SG=\'.f($row,\'SIGLA\').\'&O=L&w_sq_acordo=\'.$_REQUEST[\'w_sq_acordo\'].\'&w_menu=\'.f($row,\'menu_pai\'),$w_Imagem,$w_Imagem,f($row,\'target\')));');
          elseif ($_REQUEST['w_sq_pessoa'] > '') {
            ...
            eval ('$node' .
            i . ' = &$node1->addItem(new XNode(f($row,\'nome\'),f($row,\'LINK\').\'&P1=\'.f($row,\'P1\').\'&P2=\'.f($row,\'P2\').\'&P3=\'.f($row,\'P3\').\'&P4=\'.f($row,\'P4\').\'&TP=\'.$w_titulo.\'&SG=\'.f($row,\'SIGLA\').\'&O=\'.$O.\'&w_chave=\'.$_REQUEST[\'w_chave\'].\'&w_menu=\'.f($row,\'menu_pai\').MontaFiltro(\'GET\'),$w_Imagem,$w_Imagem,f($row,\'target\')));');
        }
    ...
    if (count($RS) > 0)
      ShowHTML('          <br>Exercício:<b>' . $_SESSION['ANO'] . '</b></TD>');
  ...
  function Vinculacao() {
    ...
    ShowHTML('</HEAD>');
    if ($O == 'I')
      BodyOpen('onLoad=document.Form.w_sq_menu_fornec.focus();');
    ...
    if ($O == 'L') {
      ...
      foreach ($RS as $row) {
        if ($i == 0)
          $w_sq_tramite = f($row, 'sq_siw_tramite');
        else
          $w_sq_tramite .= ',' . f($row, 'sq_siw_tramite');
        $i = 1;
      }
      ...
      if ($O == 'A')
        ShowHTML('<INPUT type="hidden" name="w_sq_menu_fornec" value="' . $w_sq_menu_fornec . '">');
  ...
  function Grava() {
    ...
    ScriptOpen('JavaScript');
    if (f(RS1, 'IMAGEM') > '')
      ShowHTML('  location.href=\'' .
      f($RS1, 'LINK') . '&P1=' . f($RS1, 'P1') . '&P2=' . f($RS1, 'P2') . '&P3=' . f($RS1, 'P3') . '&P4=' . f($RS1, 'P4') . '&TP=<img src=' . f($RS1, 'IMAGEM') . ' BORDER=0>' . f($RS1, 'NOME') . '&SG=' . f($RS1, 'SIGLA') . '\'; ');
    else
      ShowHTML('  location.href=\'' .
      f($RS1, 'LINK') . '&P1=' . f($RS1, 'P1') . '&P2=' . f($RS1, 'P2') . '&P3=' . f($RS1, 'P3') . '&P4=' . f($RS1, 'P4') . '&TP=<img src=' . $w_ImagemPadrao . ' BORDER=0>' . f($RS1, 'NOME') . '&SG=' . f($RS1, 'SIGLA') . '\'; ');
  ...
  function Sair() {
    ...
    if ($conLog) {
      ...
      if (!file_exists($l_caminho))
        mkdir($l_caminho);
      if (!file_exists($l_caminho . $_SESSION['P_CLIENTE']))
        mkdir($l_caminho . $_SESSION['P_CLIENTE']);
