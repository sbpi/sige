  ...
  function Inicial() {
    ...
    ShowHTML ( '</head>' );
    if ($p_regional > '' || $p_local > '' || $p_modalidade > '') {
    ...
    ShowHTML ( '          <h4 title="Selecione a Rede de Ensino!">Rede de Ensino:</h4>' );
    ...
    If (f ( $RS, 'cadprivada' ) > 0) {
      If (Nvl ( $p_rede, 'S' ) == 'S')
        ShowHTML ( '<input type="radio" name="p_rede" value="S" checked="checked" onclick="marcaTipo();"> Rede pública' );
      else
        ShowHTML ( '<input type="radio" name="p_rede" value="S" onclick="marcaTipo();"> Rede pública' );
      If ($p_rede == 'P')
        ShowHTML ( '<input type="radio" name="p_rede" value="P" checked="checked" onclick="marcaTipo();"> Rede privada' );
      else
        ShowHTML ( '<input type="radio" name="p_rede" value="P" onclick="marcaTipo();"> Rede privada' );
    } else {
    ...
    if ($p_rede == 'P') {
      ...
      selecaoRegiaoAdm ( null, null, null, $p_regional, null, 'p_regional', null, null );
    } else {
    ...
    if ($p_local == '2')
      ShowHTML ( '<input type="radio" name="p_local" value="2" checked="checked"> Rural' );
    else
      ShowHTML ( '<input type="radio" name="p_local" value="2"> Rural' );
    if ($p_local == '1')
      ShowHTML ( '&nbsp;<input type="radio" name="p_local" value="1" checked="checked"> Urbana' );
    else
      ShowHTML ( '&nbsp;<input type="radio" name="p_local" value="1"> Urbana' );
    if ($p_local == '')
      ShowHTML ( '&nbsp;<input type="radio" name="p_local" value=""  checked="checked"> Ambas' );
    else
      ShowHTML ( '&nbsp;<input type="radio" name="p_local" value="" > Ambas' );
    ShowHTML ( ' </div><div class="base"> </div>' );
    ShowHTML ( ' </div>' );
    
    ShowHTML ( ' <div id="opcoes">' );
    ShowHTML ( ' <div class="topo"> </div>' );
    selecaoEtapaModalidade ( null, null, null, $p_modalidade, $p_rede, 'p_modalidade[]', $p_rede, null ,1, $_REQUEST['p_curso'] );
    ...
    if ($p_regional > '' || $p_local > '' || $p_modalidade > '' || $_REQUEST['p_curso'] > '') {
      
      $sql = "SELECT DISTINCT 0 Tipo, sbpi.acentos(d.ds_cliente) as ordena, d.sq_cliente, d.ds_cliente, d.ds_apelido,d.ln_internet,d.no_municipio,d.sg_uf, " . $crlf . "       e.ds_logradouro,e.no_bairro,e.nr_cep, e.nr_fone_contato, d.localizacao, 'S' publica, null ds_username, " . $crlf . "       r.no_regiao " . $crlf . "  from sbpi.Cliente                               d " . $crlf . "       INNER      join sbpi.Tipo_Cliente          b ON (d.sq_tipo_cliente  = b.sq_tipo_cliente and " . $crlf . "                                                        b.tipo             = '2' " . $crlf . "                                                       ) " . $crlf . "       LEFT OUTER join sbpi.Cliente_Dados         e ON (d.sq_cliente       = e.sq_cliente) " . $crlf . "       INNER      join sbpi.Regiao_Administrativa r ON (d.sq_regiao_adm    = r.sq_regiao_adm) " . $crlf . " where d.ativo <> 'N' and d.publica = 'S' and 'P' <> '" . $p_rede . "' and d.sq_cliente = " . Nvl ( $p_regional, 0 ) . $crlf;
      if ($p_local > "")
        $sql .= "    and 0 < (select count(*) from sbpi.Cliente where sq_cliente_pai = d.sq_cliente and localizacao = " . $p_local . ") " . $crlf;
      if ($p_tipo_inst > "")
        $sql .= "    and 0 < (select count(*) from sbpi.Cliente where sq_cliente_pai = d.sq_cliente and d.sq_tipo_cliente= " . $p_tipo_inst . ") " . $crlf;
      if ($p_modalidade > "") {
        $sql .= "    and (0 < (select count(sq_cliente) from sbpi.Especialidade_Cliente where sq_cliente_pai = d.sq_cliente and to_char(sq_especialidade) in (" . $p_modalidade . ")) or " . $crlf . "         0 < (select count(*) from sbpi.Turma_Modalidade  w INNER join sbpi.Turma x ON (w.serie = x.ds_serie) INNER join sbpi.Cliente y ON (x.sq_cliente = y.sq_cliente) where y.sq_cliente_pai = d.sq_cliente and w.curso in (" . $p_modalidade . ")) " . $crlf . "        ) " . $crlf;
      }
      if ($_REQUEST["p_curso"] > ""){
            $sql .= "    and (0 < (select count(*) from sbpi.Particular_Curso w where w.sq_cliente = d.sq_cliente and w.sq_curso = (" . $_REQUEST["p_curso"] . "))) " ; 
      }
      $sql .= " UNION " . $crlf . "SELECT DISTINCT 0 Tipo, sbpi.acentos(d.ds_cliente) as ordena, d.sq_cliente, d.ds_cliente, d.ds_apelido,d.ln_internet,d.no_municipio,d.sg_uf, " . $crlf . "       e.ds_logradouro,e.no_bairro,e.nr_cep, e.nr_fone_contato, d.localizacao, 'S' publica, null ds_username, " . $crlf . "       r.no_regiao " . $crlf . "  from sbpi.Cliente                               d " . $crlf . "       LEFT OUTER join sbpi.Cliente_Dados         e ON (d.sq_cliente       = e.sq_cliente) " . $crlf . "       INNER      join sbpi.Cliente_Site          a ON (d.sq_cliente       = a.sq_cliente) " . $crlf . "       INNER      join sbpi.Tipo_Cliente          b ON (d.sq_tipo_cliente  = b.sq_tipo_cliente and " . $crlf . "                                                        b.tipo             = '2' " . $crlf . "                                                       ) " . $crlf . "       INNER      join sbpi.Cliente               c ON (d.sq_cliente       = c.sq_cliente_pai) " . $crlf . "       INNER      join sbpi.Regiao_Administrativa r ON (d.sq_regiao_adm    = r.sq_regiao_adm) " . $crlf . " where d.ativo <> 'N' and 'P' <> '" . $p_rede . "' " . $crlf;
      if ($p_local > "")
        $sql .= "    and d.localizacao    = " . $p_local . $crlf;
      if ($p_regional > "")
        $sql .= "    and c.sq_cliente_pai = " . $p_regional . $crlf;
      if ($p_tipo_inst > "")
        $sql .= "    and c.sq_tipo_cliente= " . $p_tipo_inst . $crlf;
      if ($p_modalidade > "") {
        $sql .= "    and (0 < (select count(*) from sbpi.Especialidade_Cliente w inner join sbpi.Cliente x on (w.sq_cliente = x.sq_cliente) where x.sq_cliente_pai = d.sq_cliente and to_char(w.sq_especialidade) in (" . $p_modalidade . ")) or " . $crlf . "         0 < (select count(*) from sbpi.Turma_Modalidade  w INNER join sbpi.Turma x ON (w.serie = x.ds_serie) INNER join sbpi.Cliente y ON (x.sq_cliente = y.sq_cliente) where y.sq_cliente_pai = d.sq_cliente and w.curso in (" . $p_modalidade . ")) " . $crlf . "        ) " . $crlf;
      }
      if ($_REQUEST["p_curso"] > ""){
            $sql .= "    and (0 < (select count(*) from sbpi.Particular_Curso w where w.sq_cliente = d.sq_cliente and w.sq_curso = (" . $_REQUEST["p_curso"] . "))) " ; 
      }
      
      $sql .= " UNION " . $crlf . "SELECT DISTINCT 1 Tipo, sbpi.acentos(d.ds_cliente) as ordena, d.sq_cliente, d.ds_cliente, d.ds_apelido,d.ln_internet,d.no_municipio,d.sg_uf, " . $crlf . "       f.endereco logradouro, null no_bairro, null nr_cep, null nr_fone_contato, d.localizacao, d.publica, " . $crlf . "       d.ds_username, r.no_regiao " . $crlf . "  from sbpi.Cliente                          d " . $crlf . "       INNER join sbpi.Cliente_Particular    f ON (d.sq_cliente       = f.sq_cliente) " . $crlf . "       LEFT  join sbpi.Especialidade_cliente g ON (d.sq_cliente       = g.sq_cliente) " . $crlf . "       INNER join sbpi.Regiao_Administrativa r ON (d.sq_regiao_adm    = r.sq_regiao_adm) " . $crlf . " where d.ativo <> 'N' and f.situacao = 1 and d.publica = 'N' and 'S' <> '" . $p_rede . "' " . $crlf;
      
      if ($p_local > "")
        $sql .= "    and d.localizacao    = " . $p_local . $crlf;
      if ($p_regional > "")
        $sql .= "    and d.sq_regiao_adm  = " . $p_regional . $crlf;
      if ($p_tipo_inst > "")
        $sql .= "    and d.sq_tipo_cliente= " . $p_tipo_inst . $crlf;
      if ($p_modalidade > "") {
      ...
      $sql .= " UNION " . $crlf . "SELECT DISTINCT 1 Tipo, sbpi.acentos(d.ds_cliente) as ordena, d.sq_cliente, d.ds_cliente, d.ds_apelido,d.ln_internet,d.no_municipio,d.sg_uf, " . $crlf . "       e.ds_logradouro, e.no_bairro,e.nr_cep, e.nr_fone_contato, d.localizacao, d.publica, " . $crlf . "       d.ds_username, r.no_regiao " . $crlf . "  from sbpi.Cliente                            d " . $crlf . "       INNER join sbpi.Cliente_Dados           e ON (d.sq_cliente       = e.sq_cliente) " . $crlf . "       INNER   join sbpi.Regiao_Administrativa r ON (d.sq_regiao_adm    = r.sq_regiao_adm) " . $crlf . " where d.ativo <> 'N' and d.publica = 'S' and 'P' <> '" . $p_rede . "' " . $crlf;
      
      if ($p_local > "")
        $sql .= "    and d.localizacao    = " . $p_local . $crlf;
      if ($p_regional > "")
        $sql .= "    and d.sq_cliente_pai = " . $p_regional . $crlf;
      if ($p_tipo_inst > "")
        $sql .= "    and d.sq_tipo_cliente= " . $p_tipo_inst . $crlf;
      
      if ($p_modalidade > "") {
        $sql .= "    and (0 < (select count(*) from sbpi.Especialidade_Cliente where sq_cliente = d.sq_cliente and to_char(sq_especialidade) in (" . $p_modalidade . ")) or " . $crlf . "         0 < (select count(*) from sbpi.Turma_Modalidade  w INNER join sbpi.Turma x ON (w.serie = x.ds_serie) INNER join sbpi.Cliente y ON (x.sq_cliente = y.sq_cliente) where y.sq_cliente = d.sq_cliente and w.curso in (" . $p_modalidade . ")) " . $crlf . "        ) " . $crlf;
      }
      if ($_REQUEST["p_curso"] > ""){
            $sql .= "    and (0 < (select count(*) from sbpi.Particular_Curso w where w.sq_cliente = d.sq_cliente and w.sq_curso = (" . $_REQUEST["p_curso"] . "))) " ; 
      }
      ...
      if (count ( $RS ) > 0) {
        ...
        foreach ( $RS1 as $row ) {
          if (f ( $row, 'PUBLICA' ) == 'S') {
            ...
            ShowHTML ( '                <li><a href="modelos/modpart/default.php?par=inicial&CL=' . f ( $row, 'sq_cliente' ) . '"&target="' . f( $row, 'ds_username' ) . '">' . f ( $row, 'ds_cliente' ) . '</a></b>' );
          }
          if (f ( $row, 'tipo' ) != '0') {
            ShowHTML ( '<ul>' );
            ShowHTML ( '<li>Telefone: ' . f ( $row, 'nr_fone_contato' ) . '</li>' );
            ShowHTML ( '<li>Endereço: ' . f ( $row, 'ds_logradouro' ) );
            if (Nvl ( f ( $row, 'no_bairro' ), 'nulo' ) != 'nulo')
              ShowHTML ( ' - Bairro: ' . f ( $row, 'no_bairro' ) );
            ShowHTML ( ' - ' . f ( $row, 'no_regiao' ) );
            $sql2 = "SELECT f.ds_especialidade " . $crlf . "  from sbpi.Especialidade_cliente e " . $crlf . "       INNER join sbpi.Especialidade f ON (e.sq_especialidade = f.sq_especialidade) " . $crlf . " WHERE e.sq_cliente = " . f ( $row, 'sq_cliente' ) . " " . $crlf . "UNION " . $crlf . "SELECT f.ds_especialidade " . $crlf . "  from sbpi.Especialidade f " . $crlf . " WHERE upper(ds_especialidade) like '%PROFISSIONAL%' " . $crlf . "   and 0 < (select count(sq_cliente) from sbpi.Particular_curso where sq_cliente = " . f ( $row, 'sq_cliente' ) . ") " . $crlf . "UNION " . $crlf . "select distinct ltrim(rtrim(w.curso)) ds_especialidade " . $crlf . "  from sbpi.Turma_Modalidade     w " . $crlf . "       INNER   join sbpi.Turma   x ON (w.serie = x.ds_serie) " . $crlf . "         INNER join sbpi.Cliente y ON (x.sq_cliente = y.sq_cliente) " . $crlf . " where y.sq_cliente = " . f ( $row, 'sq_cliente' ) . " " . $crlf . "ORDER BY ds_especialidade ";
