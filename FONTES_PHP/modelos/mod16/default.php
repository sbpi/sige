...
function noticia() {
  ...
  If (nvl($w_noticia,'') == ''){
     $SQL = "SELECT a.ln_externo, a.sq_noticia, a.dt_noticia data, a.ds_titulo ocorrencia, 'Escola' origem " .$crlf.
           ...
           "UNION " .$crlf.
           "SELECT a.ln_externo, a.sq_noticia, dt_noticia data, ds_titulo ocorrencia, 'Regional' origem " .$crlf.
           "from sbpi.Noticia_Cliente    a" .$crlf.
           ...
           "  and sbpi.year(a.dt_noticia) = " . $wAno .$crlf.
           "UNION " .$crlf.
           "SELECT a.ln_externo, a.sq_noticia, dt_noticia data, ds_titulo ocorrencia, 'SEDF' origem " .$crlf.
           "from sbpi.Noticia_Cliente    a" .$crlf.
           "     INNER join sbpi.Cliente b ON (a.sq_cliente = b.sq_cliente) " .$crlf.
           "     INNER join sbpi.Cliente c ON (b.sq_cliente      = c.sq_cliente_pai) " .$crlf.
  ...
  if (count($RS)>0) {
    foreach($RS as $row) {
           ...
           if(f($row,"ln_externo") != ''){
             ShowHTML ('    <TD colspan=2><a href="'.f($row,"ln_externo").'" target="_blank">' . f($row, 'ocorrencia') . '</a>&nbsp;&nbsp;&nbsp;<a href="' . $w_dir . $w_pagina . "noticia&CL=" . $CL . "&w_noticia=" . f($row, "sq_noticia") . '"><b>[Ler]</a> ');           
           }else{
