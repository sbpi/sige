...
Sub Gerencial
  ...
  If w_ea = "L" or w_ea = "W" Then
     ...
     SQL = "select a.*, " & VbCrLf & _
           ...
           "       coalesce(b.nome,'Consulta') nm_funcionalidade,   coalesce(b.codigo,0) cd_funcionalidade, " & VbCrLf & _
           ...
           "       coalesce(e.sq_cliente, d.sq_cliente) sq_secretaria, coalesce(e.ds_cliente, d.ds_cliente) nm_secretaria " & VbCrLf & _
           "  from sbpi.Cliente_Log                    a " & VbCrLf & _
           "       left outer join sbpi.Funcionalidade b on (a.sq_funcionalidade = b.sq_funcionalidade and " & VbCrLf & _
           "                                               b.tipo              = 2 " & VbCrLf & _
           "                                              ) " & VbCrLf & _
           "       inner      join sbpi.Cliente        c on (a.sq_cliente        = c.sq_cliente) " & VbCrLf & _
           "         inner   join sbpi.Tipo_Cliente    h on (c.sq_tipo_cliente   = h.sq_tipo_cliente) " & VbCrLf & _
           "         inner   join sbpi.Cliente         d on (coalesce(c.sq_cliente_pai,0)    = d.sq_cliente) " & VbCrLf & _
           "           left  join sbpi.Cliente         e on (d.sq_cliente_pai    = e.sq_cliente), " & VbCrLf & _
           "       sbpi.cliente                        f " & VbCrLf & _
           "         inner   join sbpi.Tipo_Cliente    g on (f.sq_tipo_cliente   = g.sq_tipo_cliente) " & VbCrLf & _
     ...
     If p_inicio > ""   Then SQL = SQL & "   and a.data       between to_date('" & p_inicio & "', 'dd/mm/yyyy') and to_date('" & p_fim & "', 'dd/mm/yyyy') + 1 " & VbCrLf End If
  ...
  Else
     ...
     If w_ea = "P" Then
        ...
        ShowHTML "     alert ('Informe ambas datas de recebimento ou nenhuma delas!');"
  ...
  ElseIf Instr("P",w_ea) > 0 Then
    ...
    SelecaoRegional "<u>S</u>ubordinação:", "S", "Se desejar, indique um item para repuperar apenas escolas subordinadas a ele.", p_regional, null, "p_regional", null, "onChange=""document.Form.w_ea.value='P'; document.Form.p_escola.value=''; document.Form.w_troca.value='p_escola'; document.Form.submit();"""
