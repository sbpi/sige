...
Sub showMenu
   ...
   If w_tipo = 3 Then
      ...
      SQL = "select b.ds_especialidade from sbpi.Especialidade_Cliente a inner join sbpi.Especialidade b on (a.sq_especialidade = b.sq_especialidade and a." & CL & ")"
      ConectaBD SQL
      If Not RS.EOF Then
...
Sub LogOn
  ...
  sql = "SELECT case in_destinatario when 'A' then 'Aluno' when 'P' then 'Professor' when 'E' then 'Escola' else 'Todos' end in_destinatario, " & VbCrLf & _
        ...
        "  AND a.in_destinatario = 'E'" & VbCrLf & _
        "  and sbpi.year(a.dt_arquivo) = " & wAno & VbCrLf & _
        "ORDER BY origem, a.nr_ordem, a.dt_arquivo desc, a.in_destinatario " & VbCrLf 
  ConectaBD SQL
...
Sub Valida
  ...
  Else
     ...
     SQL = "select sq_cliente, ds_username from sbpi.Cliente where upper(ds_username) = upper('" & w_uid & "') and upper(ds_senha_acesso) = '" & w_pwd & "'"
     ConectaBD SQL
...
Sub GetDocumento
  ...
  ElseIf w_ea = "L" Then
     ...
     SQL = "select * from sbpi.Cliente_Arquivo where " & replace(CL,"sq_cliente","sq_cliente") & " order by nr_ordem, ltrim(upper(ds_titulo))"
     ConectaBD SQL
  ...
  ShowHTML "  <tr><td align=""center"" colspan=""2""><font size=""1"" color=""red""><b>IMPORTANTE: <a href=""sedf/orientacoes_word.pdf"" class=""hl"" target=""_blank"">Para documentos Word, clique aqui para ler orientações sobre a formatação e a proteção do texto</a></b></font>."
  If w_ea = "L" Then
...
Sub GetEspecialidadeCliente
  ...
  If w_ea = "L" Then
     SQL = "select a.sq_especialidade, a.ds_especialidade, a.tp_especialidade, b.sq_especialidade as sq_codigo_cli " & VbCrLf & _
...
Sub GetAdmin
  ...
  SQL = "select * from sbpi.Cliente_Admin where " & CL
  ConectaBD SQL
  If RS.EOF Then
  ...
  SQL = "select a.nome, b.sq_equipamento, b.nome nm_equipamento, coalesce(c.quantidade, 0) quantidade " & VbCrLf & _
        ...
        "       left outer join sbpi.Cliente_Equipamento c on (b.sq_equipamento      = c.sq_equipamento and " & VbCrLf & _
...
Sub GetSGE
  ...
  ElseIf w_ea = "L" Then
     ' Recupera todos os registros para a listagem
     SQL = "select * from sbpi.Cliente_Foto where " & CL & " and tp_foto = 'P' order by nr_ordem"
     ConectaBD SQL
     SQL = "select * from sbpi.Cliente_Foto where " & CL & " and tp_foto = 'Q' order by nr_ordem"
...
Sub GetSGE1
  ...
  ShowHTML "         Abaixo são listadas atualizações do SGE, da mais nova até a mais antiga. Para atualizar sua versão, siga os passos abaixo:"
  ShowHTML "         <ol>"
...
Sub GetSite
  ...
  If w_ea = "A" Then
     ...
     SQL = "select a.*,a.sq_cliente, b.ds_diretorio tipo from sbpi.Cliente_Site a inner join sbpi.Modelo b on (a.sq_modelo = b.sq_modelo) where a." & CL
     ConectaBD SQL
     w_sq_cliente           = RS("sq_cliente")
  ...
  If w_ea = "A" Then
     ...
     If w_tipo <> 13 Then
        ...
        ShowHTML "       alert('Esolha arquivos com extensões \'.doc\' ou \'.pdf\'!');"
  ...
  If w_tipo = 2 Then ' Se for regional
     ...
     ShowHTML "          <br><font color=""red""><b>IMPORTANTE: <a href=""sedf/orientacoes_word.pdf"" class=""hl"" target=""_blank"">Para documentos Word, clique aqui para ler orientações sobre a formatação e a proteção do texto</a></b></font>."
     ShowHTML "      </font></td></tr>"
     ShowHTML "      <tr><td align=""center"" height=""1"" bgcolor=""#000000""></td></tr>"
...
Sub GetCalendario
  ...
  ElseIf w_ea = "L" Then
     ' Recupera todos os registros para a listagem
     'SQL = "select * from sbpi.Calendario_Cliente where " & replace(CL,"sq_cliente","sq_cliente") & " order by sbpi.year(dt_ocorrencia) desc, dt_ocorrencia"
  ...
  If InStr("IAEP",O) > 0 Then
     ...
     If InStr("IA",O) > 0 Then
        ...
        Validate "w_ds_ocorrencia", "Descrição de Ocorrência", "", "1", "2", "60", "1", "1"
        Validate "w_tipo", "Tipo de Ocorrência", "", "1", "2", "60", "1", "1"
     End If
     ShowHTML "  theForm.Botao[0].disabled=true;"
     ShowHTML "  theForm.Botao[1].disabled=true;"
     ValidateClose
...
Sub ShowLog
  ...
  If w_ea = "L" Then
     ' Recupera todos os registros para a listagem
     'SQL = "select a.*, b.nome, a.dt_ocorrencia data, b.abrangencia from sbpi.Calendario_Base a left join sbpi.Tipo_Data b on (a.sq_tipo_data = b.sq_tipo_data) order by sbpi.year(dt_ocorrencia) desc, dt_ocorrencia"
     SQL = "select * from sbpi.Cliente_Log where " & CL & " order by to_char(data,'yyyymmdd') desc, data desc"
     ConectaBD SQL
  End If
  ...
  ElseIf Instr("IAEV",O) > 0 Then
    ...
    SQL = "select * from sbpi.Cliente_Log where sq_cliente_log = " & w_chave
    ConectaBD SQL
...
Sub GetMensagem
  ...
  ElseIf w_ea = "L" Then
     ...
     SQL = "select a.*, b.no_aluno, b.nr_matricula " & VbCrLf & _
           ...
           "       inner join sbpi.Aluno b on (a.sq_aluno = b.sq_aluno and b." & replace(CL,"sq_cliente","sq_cliente") & ") " & VbCrLf & _
    ...
    If w_ea = "I" Then
       ...
       If w_texto > "" Then
          SQL = "select sq_aluno, nr_matricula, no_aluno " & VbCrLf & _
                ...
                "   and " & replace(CL,"sq_cliente","sq_cliente") & " " & VbCrLf & _
...
Public Sub Grava
  ...
  Select Case w_R
    ...
    Case conWhatDadosAdicionais
       ...
       SQL = "select count(*) existe from sbpi.Cliente_Dados where " & CL
       ConectaBD SQL
       If cInt(RS("existe")) = 0 Then
          w_chave = replace(CL,"sq_cliente=","")
          'Criação das escolas na tabela que contém dados complementares'
          SQL = "insert into sbpi.Cliente_Dados ( " & VbCrLf &_ 
    ...
    Case conWhatAdmin
       ...
       SQL = "delete sbpi.Cliente_Equipamento where " & CL & VbCrLf
       w_sql = SQL & VbCrLf
       ExecutaSQL(SQL)

       ' Atualiza os dados em sbpi.cliente_Admin
       SQL = "select count(*) existe from sbpi.Cliente_Admin where " & CL & VbCrLf
       ConectaBD SQL
       If cInt(RS("existe")) = 0 Then
          w_oper = 1
          SQL = "insert into sbpi.Cliente_Admin (sq_cliente, limpeza_terceirizada, merenda_terceirizada, banheiro) " & VbCrLf & _
       ...
       Else
          w_oper = 2
          SQL = "update sbpi.Cliente_Admin set " & VbCrLf & _
       ...
       For w_cont = 1 To Request.Form("w_equipamento").Count
          If Nvl(Request("w_equipamento")(w_cont),0) > 0 Then
             SQL = "insert into sbpi.Cliente_Equipamento (sq_cliente, sq_equipamento, quantidade) " & VbCrLf & _
                   "values (" & VbCrLf & _
                   "   " & Replace(CL,"sq_cliente=","") & ", " & VbCrLf & _
    ...
    Case conWhatDocumento
       ...
       If w_ea = "I" Then
          ...
          SQL = "select count(*) existe from sbpi.Cliente_Arquivo where ln_arquivo = '" & extractFileName(ul.Files("w_ln_arquivo").OriginalPath) & "'" & VbCrLf
          ConectaBD SQL
          If cInt(RS("existe")) > 0 Then
             ScriptOpen "JavaScript"
       ...
       ElseIf w_ea = "A" Then
          ' Verifica se o nome desse arquivo já existe
          SQL = "select count(*) existe from sbpi.Cliente_Arquivo where ln_arquivo = '" & extractFileName(ul.Files("w_ln_arquivo").OriginalPath) & "' and sq_arquivo <> " & ul.Form("w_chave") & VbCrLf
          ConectaBD SQL
          If cInt(RS("existe")) > 0 Then
             ScriptOpen "JavaScript"
             ShowHTML "  alert('Nome do arquivo já existe!');"
       ...
       ElseIf w_ea = "E" Then
          ' Remove o arquivo físico
          SQL = "select ln_arquivo from sbpi.Cliente_Arquivo where sq_arquivo = " & ul.Form("w_chave")
          ConectaBD SQL
          DeleteAFile w_diretorio & RS("ln_arquivo")
          DesconectaBD

          SQL = " delete sbpi.Cliente_Arquivo where sq_arquivo = " & ul.Form("w_chave") & VbCrLf
          ExecutaSQL(SQL)
    ...
    Case conWhatCalendario
       ...
       If w_ea = "I" Then
          ' Recupera o valor da próxima chave primária
          SQL = "select sbpi.sq_ocorrencia.sbpi chave from sbpi.Calendario_Cliente" & VbCrLf
          ConectaBD SQL
          w_chave = RS("chave")
          DesconectaBD
    ...
    Case conWhatNotCliente
       ...
       If w_ea = "I" Then
          ' Recupera o valor da próxima chave primária
          SQL = "select sbpi.sq_noticia.sbpi chave from sbpi.Noticia_Cliente" & VbCrLf
          ConectaBD SQL
          w_chave = RS("chave")
          DesconectaBD
    ...
    Case conWhatMensagem
       ...
       If w_ea = "I" Then
          ' Recupera o valor da próxima chave primária
          SQL = "select sbpi.sq_mensagem),0)+1 chave from sbpi.Mensagem_Aluno" & VbCrLf
          ConectaBD SQL
          w_chave = RS("chave")
          DesconectaBD
    ...
    Case conWhatSGE
       ...
       If w_ea = "I" Then
          ' Verifica se o nome desse arquivo já existe
          SQL = "select count(*) existe from sbpi.Cliente_Foto where ln_foto = '" & extractFileName(ul.Files("w_ln_foto").OriginalPath) & "'" & VbCrLf
          ConectaBD SQL
          If cInt(RS("existe")) > 0 Then
             ScriptOpen "JavaScript"
             ShowHTML "  alert('Nome do arquivo já existe!');"
          ...
          ' Recupera o valor da próxima chave primária
          SQL = "select sbpi.sq_cliente_foto.nextval chave from sbpi.Cliente_Foto" & VbCrLf
          ConectaBD SQL
          w_chave = RS("chave")
          DesconectaBD

          ' Insere o arquivo
          SQL = " insert into sbpi.Cliente_Foto " & VbCrLf & _
       ...
       ElseIf w_ea = "A" Then
          ' Verifica se o nome desse arquivo já existe
          SQL = "select count(*) existe from sbpi.Cliente_Foto where ln_foto = '" & extractFileName(ul.Files("w_ln_foto").OriginalPath) & "' and sq_cliente_foto <> " & ul.Form("w_chave") & VbCrLf
          ConectaBD SQL
          If cInt(RS("existe")) > 0 Then
             ScriptOpen "JavaScript"
             ShowHTML "  alert('Nome do arquivo já existe!');"
          ...
          ' Remove o arquivo físico
          SQL = "select ln_foto foto from sbpi.Cliente_Foto where sq_cliente_foto = " & ul.Form("w_chave")
          ConectaBD SQL
          w_foto = RS("foto")
          DesconectaBD

          SQL = " update sbpi.Cliente_Foto set " & VbCrLf & _
       ...
       ElseIf w_ea = "E" Then
          ' Remove o arquivo físico
          SQL = "select ln_foto from sbpi.Cliente_Foto where sq_cliente_foto = " & ul.Form("w_chave")
          ConectaBD SQL
          DeleteAFile w_diretorio & RS("ln_foto")
          DesconectaBD

          SQL = " delete sbpi.Cliente_Foto where sq_cliente_foto = " & ul.Form("w_chave") & VbCrLf
          ExecutaSQL(SQL)
