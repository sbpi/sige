<?php
  ...
  function RetornaFormulario($l_troca = null, $l_sg = null, $l_menu = null, $l_o = null, $l_dir = null, $l_pagina = null, $l_par = null, $l_p1 = null, $l_p2 = null, $l_p3 = null, $l_p4 = null, $l_tp = null, $l_r = null) {
    ...
    if (nvl($l_troca, 'x') == 'w_assinatura') {
      ...
      if ($w_resultado > '')
        ShowHTML('  alert(\'ATENÇÃO: erro no registro do log.\n' . $w_resultado . '\');');
    }
  ...
  function SelecaoOperacao($label, $accesskey, $hint, $chave, $chaveAux, $campo, $restricao, $atributo) {
    ...
    If (Nvl($chave, "") == "1")
      ShowHTML('          <option value="1" SELECTED>Inclusão');
    Else
      ShowHTML('          <option value="1">Inclusão');
    If (Nvl($chave, "") == "2")
      ShowHTML('          <option value="2" SELECTED>Alteração');
    Else
      ShowHTML('          <option value="2">Alteração');
    If (Nvl($chave, "") == "3")
      ShowHTML('          <option value="3" SELECTED>Exclusão');
    Else
      ShowHTML('          <option value="3">Exclusão');
    ShowHTML('          </select>');
  }
  ...
  function SelecaoBlocoDados($label, $accesskey, $hint, $chave, $chaveAux, $campo, $restricao, $atributo) {
    ...
    If (Nvl($chave, "") == "133")
      ShowHTML('          <option value="133" SELECTED>Calendário');
    Else
      ShowHTML('          <option value="133">Calendário');
    If (Nvl($chave, "") == "220")
      ShowHTML('          <option value="220" SELECTED>Dados adicionais');
    Else
      ShowHTML('          <option value="220">Dados adicionais');
    If (Nvl($chave, "") == "123")
      ShowHTML('          <option value="123" SELECTED>Dados básicos');
    Else
      ShowHTML('          <option value="123">Dados básicos');
    If (Nvl($chave, "") == "144")
    ...
    If (Nvl($chave, "") == "222")
      ShowHTML('          <option value="222" SELECTED>Notícias');
    Else
      ShowHTML('          <option value="222">Notícias');
    ShowHTML("          </select>");
  }
  ...
  function ExibeSmile($l_tipo, $l_andamento, $l_legenda = 0) {
    ...
    if ($l_legenda) {
      ...
      if ($l_tipo == 'IDE') {
        if ($l_andamento < 70)
          $l_string .= '<img title="IDE fora da faixa desejável." src="' . $conRootSIW . $conImgSmAtraso . '" border=0 width="10" height="10">';
        elseif ($l_andamento < 90 || $l_andamento > 120) $l_string .= '<img title="IDE próximo da faixa desejável." src="' . $conRootSIW . $conImgSmAviso . '" border=0 width="10" height="10">';
          ...
          $l_string .= '<img title="IDE na faixa desejável." src="' . $conRootSIW . $conImgSmNormal . '" border=0 width="10" height="10">';
      }
      elseif ($l_tipo == 'IDC') {
        if ($l_andamento < 70)
          $l_string .= '<img title="IDC fora da faixa desejável." src="' . $conRootSIW . $conImgSmAtraso . '" border=0 width="10" height="10">';
        elseif ($l_andamento < 90 || $l_andamento > 120) $l_string .= '<img title="IDC próximo da faixa desejável." src="' . $conRootSIW . $conImgSmAviso . '" border=0 width="10" height="10">';
          ...
          $l_string .= '<img title="IDC na faixa desejável." src="' . $conRootSIW . $conImgSmNormal . '" border=0 width="10" height="10">';
      }
    }
    return $l_string;
  }
  ...
  function ExibeSinalPesquisa($l_legenda, $l_inicio, $l_fim, $l_dias_aviso = 0) {
    ...
    if ($l_legenda) {
      ...
      elseif ($l_dias_aviso <= time()) $l_string .= '<img title="Pesquisa com ' . f($RS_Parametro, 'dias_aviso_pesquisa') . ' dias ou menos de validade." src="' . $conRootSIW . $conImgSmAviso . '" border=0 width="10" height="10" align="center">';
        ...
        $l_string .= '<img title="Pesquisa válida." src="' . $conRootSIW . $conImgSmNormal . '" border=0 width="10" height="10" align="center">';
    }
    return $l_string;
  }
  ...
  function exibeImagemAnexo($l_exibe = 0) {
    ...
    $l_string = '';
    if ($l_exibe > 0)
      $l_string .= '<img title="Há arquivos disponíveis para download." src="' . $conRootSIW . $conImgDownload . '" border=0 width="14" height="14" align="center">';
    return $l_string;
  }
