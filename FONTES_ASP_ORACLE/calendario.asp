...
REM Descricao: Manutenção do calendário das escolas privadas
REM Home     : http://www.sbpi.com.br/
REM Criacao  : 28/09/2008 14:20
REM Autor    : SBPI
REM Versao   : 1.0.0.0
...
REM -------------------------------------------------------------------------
  ...
  ElseIf Session("Username") > "" and  CL > "" Then
     ...
     SQL = "select b.tipo from sbpi.Cliente a inner join sbpi.Tipo_Cliente b on (a.sq_tipo_cliente = b.sq_tipo_cliente and a." & CL & ")"
     ConectaBD SQL
     w_tipo = RS("tipo")
     DesconectaBD
     Main
...
Sub showMenu
   ...
   ShowHTML "<title>SIGE-WEB - Calendário</title>"
   ShowHTML "   <link href=""/sige_ora/css/particular.css"" media=""screen"" rel=""stylesheet"" type=""text/css"" />"
   ShowHTML "   <link href=""/sige_ora/css/impressao.css"" media=""print"" rel=""stylesheet"" type=""text/css"" />"
   ShowHTML "<style>"
   ShowHTML "<// a { color: ""#000000""; text-decoration: ""none""; } "
   ShowHTML "    a:hover { color:""#000000""; text-decoration: ""underline""; }"
   ShowHTML "    .SS{text-decoration:none;} "
...
'  sql = "SELECT case in_destinatario when 'A' then 'Aluno' when 'P' then 'Professor' when 'E' then 'Escola' else 'Todos' end in_destinatario, " & VbCrLf & _
'        "       dt_arquivo, ds_titulo, nr_ordem, ds_arquivo, ln_arquivo, 'SEDF' Origem, x.ln_internet diretorio " & VbCrLf & _
'        "from sbpi.Cliente_Arquivo a INNER join sbpi.Cliente x ON (a.sq_cliente = x.sq_cliente)" & VbCrLf & _
'        "WHERE ativo = 'S'" & VbCrLf & _
'        "  AND x.sq_cliente = 0" & VbCrLf & _
'        "  AND in_destinatario = 'E'" & VbCrLf & _
'        "  and sbpi.year(a.dt_arquivo) = " & wAno & VbCrLf & _
...
'  SQL = "SELECT sbpi.year(dt_arquivo) ano " & VbCrLf & _
'        "from sbpi.Cliente_Arquivo a INNER join sbpi.Cliente x ON (a.sq_cliente = x.sq_cliente)" & VbCrLf & _
'        "WHERE ativo = 'S'" & VbCrLf & _
'        "  AND x.sq_cliente = 0" & VbCrLf & _
'        "  AND in_destinatario = 'E'" & VbCrLf & _
...
Sub Valida
  ...
  Else
     ...
     SQL = "select * from sbpi.Cliente where upper(ds_username) = upper('" & w_uid & "') and ds_senha_acesso = '" & w_pwd & "'"
     ConectaBD SQL
...
Public Sub Grava
  ...
  If w_R <> conWhatSGE AND w_R <> "CALS" Then
     SQL = "select sq_funcionalidade from sbpi.Funcionalidade where tipo = 2 and codigo = '" & w_R & "'"
     ConectaBD SQL
...
Select Case w_R
  ...
  Case conWhatCalendario
       ...
       If w_ea = "I" Then
        Dim Item
           For each Item in Request("checkbox")
               ...
               SQL = "select count(*) basecal from sbpi.Calendario_Base where dt_ocorrencia = to_date('" & FormataDataEdicao(FormatDateTime(Request("w_dt_ocorrencia"),2)) & "','dd/mm/yyyy') and sbpi.year(dt_ocorrencia) = " & year(Request("w_dt_ocorrencia"))
               ConectaBD SQL
               ...
               SQL = " insert into sbpi.Calendario_Cliente " & VbCrLf & _
                   "    (sq_ocorrencia, sq_cliente, dt_ocorrencia, ds_ocorrencia, sq_tipo_data, sq_particular_calendario, dia_letivo) " & VbCrLf & _
                   " (select " & w_chave & ", " & VbCrLf & _
                   ...
                   " from sbpi.Tipo_Data where sq_tipo_data = " & Request("w_tipo") & " )" & VbCrLf
       ...
       ElseIf w_ea = "A" Then
          ...
          SQL = "select sq_particular_calendario calendario from sbpi.Calendario_Cliente where sq_ocorrencia = " & Request("w_chave")
          ConectaBD SQL
          Dim IdCalendario
          IdCalendario = RS("calendario")
          DesconectaBD        
          
          SQL = "select count(sbpi.ValidaCalendario(" & IdCalendario & "," & year(Request("w_dt_ocorrencia")) & ")) Restricao "
          ConectaBD SQL          
          
          IF(RS("restricao") > 0 ) Then          
          SQL = " update sbpi.Particular_Calendario set " & VbCrLf & _
                "     ultima_alteracao          = sysdate, " & VbCrLf & _
                "     restricao                 = 'S' " & VbCrLf & _
                "where sq_particular_calendario = " & IdCalendario & VbCrLf
                ExecutaSQL(SQL)
          else
          SQL = " update sbpi.Particular_Calendario set " & VbCrLf & _
                "     ultima_alteracao          = sysdate," & VbCrLf & _
                "     restricao                 = 'N' " & VbCrLf & _
                "where sq_particular_calendario = " & IdCalendario & VbCrLf
                ExecutaSQL(SQL)          
          End If
          DesconectaBD        
          ...
          SQL = " update sbpi.Calendario_Cliente set " & VbCrLf & _
                ...
                "     dia_letivo               = (select dia_letivo from sbpi.Tipo_Data where sq_tipo_data = " & Request("w_tipo") & ")"& VbCrLf & _
       ...
       SQL = "update sbpi.Cliente set dt_alteracao = sysdate where " & CL & VbCrLf
       ExecutaSQL(SQL)
       dbms.CommitTrans()
...
Case "CALS"
       ...
       If w_ea = "I" Then
          ' Recupera o valor da próxima chave primária
          SQL = "select sq_particular_calendario.nextval chave from sbpi.Particular_Calendario" & VbCrLf
          ConectaBD SQL
          w_chave = RS("chave")
          DesconectaBD

          ' Insere o arquivo
          SQL = " insert into sbpi.Particular_Calendario " & VbCrLf & _
       ...
       ElseIf w_ea = "E" Then
          SQL = " select count(*) registros from sbpi.Calendario_Cliente where sq_particular_calendario = " & Request("w_chave")
          ConectaBD SQL
          If RS("registros") = 0 Then          
            SQL = " delete sbpi.Particular_Calendario where sq_particular_calendario = " & Request("w_chave") & VbCrLf
            ExecutaSQL(SQL)
          Else
            %>
...
Sub ShowCurso
  ...
  End If
    ...
    sql = "SELECT b.ds_curso,  " & VbCrLf & _
           ...
           "       end nm_categoria " & VbCrLf & _ 
           "  from sbpi.Particular_curso a " & VbCrLf & _ 
           "       inner join sbpi.Curso b on (a.sq_curso = b.sq_curso)" & VbCrLf & _ 
           " WHERE a." & CL
...
' Novo Bloco de Dados
  ...
  sql = "SELECT b.ds_cliente, a.cnpj_escola, a.mantenedora, a.mantenedora_endereco, a.cnpj_executora, a.codinep, a.diretor, a.secretario, a.aut_hab_secretario, " & VbCrLf & _
        "       coalesce(to_char(a.vencimento,'dd/mm/yyyy'),'Sem informação') vencimento, " & VbCrLf & _
        "       a.endereco, b.no_municipio, b.sg_uf,  " & VbCrLf & _
        "       '(' || substr(telefone_1,0,3)||')'|| substr(telefone_1,3,110) telefone_1, " & VbCrLf & _
        "       '(' || substr(telefone_2,0,3)||')'|| substr(telefone_2,3,110) telefone_2, " & VbCrLf & _
        "       a.email_1, a.email_2, a.cep, " & VbCrLf & _
        "       '(' || substr(fax,0,3)||')'|| substr(fax,3,110) fax, " & VbCrLf & _ 
        "       b.localizacao, c.no_regiao, a.url " & VbCrLf & _ 
        "from sbpi.cliente_particular                 a " & VbCrLf & _
        "     INNER   join sbpi.Cliente               b on (a.sq_cliente   = b.sq_cliente) " & VbCrLf & _
        "       INNER join sbpi.Regiao_Administrativa c on (b.sq_regiao_adm = c.sq_regiao_adm) " & VbCrLf & _
        " where a." & CL
...
'Fim do Novo Bloco
  ...
  End If
     ...
     sql = "SELECT INFANTIL, FUNDAMENTAL, EJA, MEDIO, DISTANCIA, PROFISSIONAL " &_
           "from sbpi.cliente_particular a " & _
           "WHERE a." & CL
     dim modalidade
...
Sub ShowPortaria
  Dim sql, strNome, wCont, wRede, wAno
    ...
    sql = "SELECT numero, data, dodf, dodf_pagina, dodf_data, observacao from sbpi.Particular_Portaria a WHERE a." & CL
    ConectaBD sql
    Cabecalho
    ShowHTML "<head>"
    ShowHTML "   <meta http-equiv=""Content-Type"" content=""text/html; charset=iso-8859-1"" /> "
...
Sub ShowOS
  ...
  Dim sql, strNome, wCont, wRede, wAno
    ...
    sql = "select  numero, data, dodf, dodf_pagina, dodf_data, observacao from sbpi.particular_os a WHERE a." & CL
    ConectaBD sql
    Cabecalho
    ShowHTML "<head>"
    ShowHTML "   <meta http-equiv=""Content-Type"" content=""text/html; charset=iso-8859-1"" /> "
    ...
    If Not RS.EOF Then
        ShowHTML "<p align=""justify"">Informações sobre Ordens de Serviço da Instituição:</p>"
...
Public Sub FormCal
  ...
  If nvl(Request("w_imprime"),"N") = "N" Then
     Response.Write "<BODY topmargin=0 bgcolor=""#FFFFFF"" BGPROPERTIES=""FIXED"" text=""#000000"" link=""#000000"" vlink=""#000000"" alink=""#FF0000"" BACKGROUND=""img/background.gif"">"
     
     If(Request("CL") > "") Then
        SQL = "select ds_cliente nome from sbpi.Cliente where " & CL
        ConectaBD SQL
        ShowHTML "<H3>" & RS("nome") & "</H3>"
     Else
     ...
     SQL = "select distinct(sbpi.year(dt_ocorrencia)) ano from sbpi.Calendario_Cliente where " & replace(CL,"sq_cliente","sq_cliente") & "order by sbpi.year(dt_ocorrencia) desc"
     ConectaBD SQL
     ...
     If 1=1 then'Request("w_ano") <> "" Then           
         SQL = "select distinct(b.nome), a.sq_particular_calendario, b.ordem from sbpi.Calendario_Cliente a left join sbpi.Particular_Calendario b on (a.sq_particular_calendario = b.sq_particular_calendario) where a." & Replace(CL, "sq_cliente", "sq_cliente") & " and sbpi.year(dt_ocorrencia) = " & nvl(Request("w_ano"),0) & " order by b.ordem"
         ConectaBD SQL
  ...
  sql = "SELECT b.ds_cliente, a.cnpj_escola, a.mantenedora, a.mantenedora_endereco, a.cnpj_executora, a.codinep, a.diretor, a.secretario, a.aut_hab_secretario, " & VbCrLf & _
        ...
        "       a.email_1, a.email_2, a.cep, " & VbCrLf & _
        "       '(' || substr(fax,0,3)||')'|| substr(fax,3,110) fax, " & VbCrLf & _ 
        "       b.localizacao, c.no_regiao, a.url, d.nome nomecal, d.observacao " & VbCrLf & _ 
        "from sbpi.cliente_particular                 a " & VbCrLf & _
        "     INNER   join sbpi.Cliente               b on (a.sq_cliente   = b.sq_cliente) " & VbCrLf & _
        "       INNER join sbpi.Regiao_Administrativa c on (b.sq_regiao_adm = c.sq_regiao_adm) " & VbCrLf & _
        "       INNER join sbpi.Particular_Calendario d on (b.sq_cliente = d.sq_cliente) " & VbCrLf & _
  ...
  Else
     ...
     ShowHTML "  <b>"  & RS("ds_cliente") & "</b>"
     ShowHTML "  <br/>"  & RS("endereco")
     ShowHTML "  <br/>"  & RS("mantenedora")
     ShowHTML "  <br/>Credenciamento: " & RS("vencimento")
     ShowHTML "  <br/>Oferta: " & RS("nomecal")  
     ShowHTML "  <br/>CALENDÁRIO ESCOLAR " & Request("w_ano")
  End If
  
  DesconectaBD

  sql = "SELECT '' cor, b.imagem, a.dt_ocorrencia data, a.ds_ocorrencia ocorrencia, 'B' origem from sbpi.Calendario_base a left join sbpi.Tipo_Data b on (a.sq_tipo_data = b.sq_tipo_data) WHERE sbpi.year(dt_ocorrencia)=" & Request("w_ano") & " " & VbCrLf & _
        "UNION " & VbCrLf & _
        "SELECT '#99CCFF' cor, b.imagem, a.dt_ocorrencia data, a.ds_ocorrencia ocorrencia, 'E' origem from sbpi.Calendario_Cliente a left join sbpi.Tipo_Data b on (a.sq_tipo_data = b.sq_tipo_data) WHERE sq_cliente = " & w_cliente & "  AND sbpi.year(dt_ocorrencia)= " & Request("w_ano") & " AND sq_particular_calendario = " & Request("w_calendario") & VbCrLf & _
  ...
  If sstrIN = 0 then
     ...
     If nvl(Request("w_imprime"),"N") = "N" Then
        ...
        ShowHTML "  <td>"
     Else
        ShowHTML "<tr><td><TABLE align=""center"" border=0>"
        ShowHTML "<tr><td align=""center""><TABLE border=1 cellSpacing=0 cellpadding=0>"
        ShowHTML "<tr valign=""top"">"     
        ShowHTML "  <td>" & MontaCalendario("01" & wAno, wDatas, wCores, wImagem)
        ShowHTML "  <td>" & MontaCalendario("02" & wAno, wDatas, wCores, wImagem)
        ShowHTML "  <td>" & MontaCalendario("03" & wAno, wDatas, wCores, wImagem)
     ...
     sql = "  select * from " & VbCrLf & _
           ...
           "           from sbpi.Calendario_Cliente a inner join sbpi.Tipo_Data b on (a.sq_tipo_data = b.sq_tipo_data and sbpi.year(a.dt_ocorrencia) = " &  Request("w_ano") &  " and b.sigla = 'IA') " & VbCrLf & _
           "          where sq_cliente = " & w_cliente & VbCrLf & _
           "            and a.sq_particular_calendario = " & Request("w_calendario") & VbCrLf & _
           "        ) a, " & VbCrLf & _
           "        (select dt_ocorrencia w_let_fim " & VbCrLf & _
           "           from sbpi.Calendario_Cliente a inner join sbpi.Tipo_Data b on (a.sq_tipo_data = b.sq_tipo_data and sbpi.year(a.dt_ocorrencia) = " &  Request("w_ano") &  " and b.sigla = 'TA') " & VbCrLf & _
           "          where sq_cliente = " & w_cliente & VbCrLf & _
           "            and a.sq_particular_calendario = " & Request("w_calendario") & VbCrLf & _
           "        ) b, " & VbCrLf & _
           "        (select dt_ocorrencia w_let2_ini " & VbCrLf & _
           "           from sbpi.Calendario_Cliente a inner join sbpi.Tipo_Data b on (a.sq_tipo_data = b.sq_tipo_data and sbpi.year(a.dt_ocorrencia) = " &  Request("w_ano") &  " and b.sigla = 'I2') " & VbCrLf & _
           "          where sq_cliente = " & w_cliente & VbCrLf & _
           "            and a.sq_particular_calendario = " & Request("w_calendario") & VbCrLf & _
           "        ) c, " & VbCrLf & _
           "        (select dt_ocorrencia w_let1_fim " & VbCrLf & _
           "           from sbpi.Calendario_Cliente a inner join sbpi.Tipo_Data b on (a.sq_tipo_data = b.sq_tipo_data and sbpi.year(a.dt_ocorrencia) = " &  Request("w_ano") &  " and b.sigla = 'T1') " & VbCrLf & _
           "          where sq_cliente = " & w_cliente & VbCrLf & _
           "            and a.sq_particular_calendario = " & Request("w_calendario") & VbCrLf & _
           "        ) d " & VbCrLf
     ConectaBD sql
     ...
     For w_cont = 1 to 2
        ...
        For wCont = w_ini to w_fim
           ...
           Else
              sql = "SELECT last_Day(to_date('01/" & mid(100+wCont,2,2) & "/'||sbpi.year(sysdate),'dd/mm/yyyy')) fim from dual" & VbCrLf 
              ConectaBD SQL
              w_final = RS("fim")
              DesconectaBD
           End If

           sql = "SELECT sbpi.diasLetivos('" & w_inicial & "', '" & w_final & "'," & w_cliente & ","& Request("w_calendario") &") qtd from dual" & VbCrLf 
'           Response.Write sql
           ConectaBD SQL
           If cInt(RS("qtd")) > 0 Then
              ShowHTML "  <TR>"
              ShowHTML "    <TD>" & nomeMes(wCont)
              ShowHTML "    <TD ALIGN=""CENTER"">" & cInt(RS("qtd"))
              w_mes = cInt(RS("qtd"))

              DesconectaBD
              sql = "SELECT count(*) qtd from sbpi.Calendario_Cliente a inner join sbpi.Tipo_Data b on (a.sq_tipo_data = b.sq_tipo_data and b.sigla ='SL') WHERE sq_particular_calendario = " &  Request("w_calendario") & " and sq_cliente = " & w_cliente & "  AND dt_ocorrencia between to_date('" & w_inicial & "','dd/mm/yyyy') and to_date('" & w_final & "','dd/mm/yyyy')" & VbCrLf
              ConectaBD sql
              ShowHTML "    <TD ALIGN=""CENTER"">" & cInt(RS("qtd"))
              w_mes = w_mes + cInt(RS("qtd"))
              DesconectaBD
     ...
     ShowHTML "<tr valign=""top"">"
     'Legenda
     sql = "SELECT * from sbpi.Tipo_Data where abrangencia <> 'P' order by nome " & VbCrLf
     ConectaBD sql
     If Not RS.EOF Then
        If nvl(Request("w_imprime"),"N") = "N" Then
     ...
     DesconectaBD
     'Feriados
     sql = "SELECT '' cor, b.imagem, a.dt_ocorrencia data, a.ds_ocorrencia ocorrencia, 'B' origem from sbpi.Calendario_base a left join sbpi.Tipo_Data b on (a.sq_tipo_data = b.sq_tipo_data) WHERE sbpi.year(dt_ocorrencia)=" & Request("w_ano") & " AND b.sigla <> 'CN' " & VbCrLf & _
           "ORDER BY data, origem desc, ocorrencia" & VbCrLf
     ConectaBD sql
     If Not RS.EOF Then
        ShowHTML "  <td><TABLE width=""90%"" align=""center"" border=0 cellSpacing=1>"
     ...
     'Recesso
     sql = "SELECT '#99CCFF' cor, b.imagem, a.dt_ocorrencia data, a.ds_ocorrencia ocorrencia, 'E' origem from sbpi.Calendario_Cliente a inner join sbpi.Tipo_Data b on (a.sq_tipo_data = b.sq_tipo_data and b.sigla in ('RE','RA')) WHERE sq_cliente = " & w_cliente & " AND sbpi.year(dt_ocorrencia)= " & Request("w_ano") & " AND sq_particular_calendario = " & Request("w_calendario") &  VbCrLf & _
           "ORDER BY data, origem desc, ocorrencia" & VbCrLf
     ConectaBD sql
     ...
     wCont = 0
     sql = "SELECT '#99CCFF' cor, b.imagem, a.dt_ocorrencia data, a.ds_ocorrencia ocorrencia, 'E' origem from sbpi.Calendario_Cliente a inner join sbpi.Tipo_Data b on (a.sq_tipo_data = b.sq_tipo_data and b.sigla ='SL') WHERE sq_cliente = " & w_cliente & "  AND sbpi.year(dt_ocorrencia)= " & Request("w_ano") & " AND sq_particular_calendario = " & Request("w_calendario") &  VbCrLf & _
           "ORDER BY data, origem desc, ocorrencia" & VbCrLf 
     ConectaBD sql
  ...
  If Request("w_ocorrencia")>"" Then
    SQL = "select 'BASE' origem, NULL sq_particular_calendario, a.sq_ocorrencia, a.dt_ocorrencia, a.ds_ocorrencia, a.sq_tipo_data, b.nome " & VbCrLf & _
          "  from sbpi.Calendario_Base       a " & VbCrLf & _
          "       left join sbpi.Tipo_Data   b on (a.sq_tipo_data = b.sq_tipo_data) " & VbCrLf & _
          " where sbpi.year(a.dt_ocorrencia) = " & Request("w_ano") & VbCrLf & _
          "UNION " & VbCrLf & _
          ...
          "  from sbpi.Calendario_Cliente              a " & VbCrLf & _
          "       left join sbpi.Tipo_Data             b on (a.sq_tipo_data             = b.sq_tipo_data) " & VbCrLf & _
          "       left join sbpi.Particular_Calendario c on (a.sq_particular_calendario = c.sq_particular_calendario) " & VbCrLf & _
          " where sbpi.year(a.dt_ocorrencia) = " & Request("w_ano") & " and c.sq_particular_calendario = " & Request("w_calendario") & VbCrLf & _
    ...
    Else
      SQL = "select sbpi.ValidaCalendario(" & Request("w_calendario") & "," & Request("w_ano") & ") Validacao from dual "
      RS1.Open SQL, DBMS
...
DesconectaBD
  ...
  ElseIf w_ea = "L" Then
     ...
     SQL = "select 'BASE' origem, c.sq_particular_calendario, c.homologado, a.sq_ocorrencia, a.dt_ocorrencia, a.ds_ocorrencia, a.sq_tipo_data, b.nome, c.nome nomecal, sbpi.year(dt_ocorrencia) ano" & VbCrLf & _
           "  from sbpi.Calendario_Base       a " & VbCrLf & _
           "       left join sbpi.Tipo_Data   b on (a.sq_tipo_data = b.sq_tipo_data), " & VbCrLf & _
           "       escParticular_Calendario c " & VbCrLf & _
           " where sbpi.year(a.dt_ocorrencia) >= 2009 and c." & CL & "  " & VbCrLf & _
           "UNION " & VbCrLf & _
           "select 'ESCOLA' origem, c.sq_particular_calendario, c.homologado, a.sq_ocorrencia, a.dt_ocorrencia, a.ds_ocorrencia, a.sq_tipo_data, b.nome, c.nome nomecal, sbpi.year(dt_ocorrencia) ano " & VbCrLf & _
           "  from sbpi.Calendario_Cliente              a " & VbCrLf & _
           "       left join sbpi.Tipo_Data             b on (a.sq_tipo_data             = b.sq_tipo_data) " & VbCrLf & _
           "       left join sbpi.Particular_Calendario c on (a.sq_particular_calendario = c.sq_particular_calendario) " & VbCrLf & _
           " where a." & replace(CL,"sq_cliente","sq_cliente") & "  " & VbCrLf & _
  ...
  ShowHTML "<div id=""conteudo"" align=center><center>"
  ShowHTML "<table border=""0"" cellpadding=""0"" cellspacing=""0"" width=""95%"">"
  If w_ea = "L" Then
    ' Exibe a quantidade de registros apresentados na listagem e o cabeçalho da tabela de listagem
    ShowHTML "<li title=""Calendário Incompleto ou possui restrições."">Restrições no Calendário</li><br/>"
        ...
        'While Calendario = RS("nomecal")
            ...
            if Calendario <> RS("nomecal") Then
                If (Instr(RS("homologado"),"S")) Then
                   ShowHTML "      <tr bgcolor=""#fffff1"" valign=""top""><TD colspan=4 align=""left""><font size=2><B>" & RS("nomecal") & "</b><font color=""#ff3737""> (Calendário homologado pela SEDF)</font>"
                Else
                   ShowHTML "      <tr bgcolor=""#fffff1"" valign=""top""><TD colspan=4 align=""left""><font size=2><B>" & RS("nomecal") & "</b>"
                End If                
                SQL = "select sbpi.ValidaCalendario(" & RS("sq_particular_calendario") & "," & wAno & ") Validacao from dual "
  ...
  ElseIf Instr("IAEV",O) > 0 Then
    ...
    SQL = "SELECT * from sbpi.Tipo_Data a WHERE a.abrangencia <> 'P' AND a.sigla <> 'CN' AND a.sigla <> 'FF' ORDER BY a.nome" & VbCrLf
    ConectaBD SQL
    While Not RS.EOF
    ...
    If w_ea <> "A" Then
        ...
        ShowHTML "          </td></tr>"
    Else
        SQL = " select c.homologado, c.nome nomecal " & VbCrLf & _ 
              " from sbpi.Calendario_Cliente a " & VbCrLf & _ 
              " left join  escParticular_Calendario c on (a.sq_particular_calendario = c.sq_particular_calendario)" & VbCrLf & _ 
              " where sq_cliente=" & replace(CL,"sq_cliente=","") & " and sq_ocorrencia = " & Request("w_chave") 
        ConectaBD SQL
        ShowHTML "          <tr><td>"
...
Sub GetCalendarios
  ...
  ElseIf w_ea = "L" Then
     ' Recupera todos os registros para a listagem
     SQL = "select * from sbpi.Particular_Calendario where " & CL & "order by ordem, nome"
     ConectaBD SQL
  ElseIf InStr("AEV",w_ea) > 0 and w_Troca = "" Then
     ' Recupera os dados do endereço informado
