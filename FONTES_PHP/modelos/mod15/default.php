  ...
  include_once ($w_dir_volta . 'funcoes/selecaoEtapaModalidade.php');
  ...
  ShowHTML('      <li><a ' . (($par == 'PROJETO') ? 'class="selected"' : '') . ' href="' . $w_dir . $w_pagina . 'projeto&CL=' . $CL . '&O=1" >Composição Administrativa</a></li>');
  ...
  ShowHTML('      <li><a ' . (($par == 'ESCOLA') ? 'class="selected"' : '') . ' href="' . $w_dir . $w_pagina . 'escola&CL=' . $CL . '" >Escolas</a></li>');
  ...
  function escola() {
    extract($GLOBALS);
    $strAction = $w_dir . $w_pagina . 'Escola&CL=' . $CL . '&O=0';
    ShowHTML(' <h3>Busca avançada</h3>');
    ShowHTML(' <!-- <form action=" id="formConteudo">-->');
    ShowHTML(' <FORM ACTION="' . $strAction . '" id="form1" name="form1" METHOD="POST">');
    ...
    if ($p_regiao > '' || $p_regional > '' || $p_local > '' || $p_modalidade > '') {
      ...
      $SQL = "SELECT DISTINCT 1 Tipo, sbpi.acentos(d.ds_cliente) as ordena, d.sq_cliente, d.ds_cliente, d.ds_apelido,d.ln_internet,d.no_municipio,d.sg_uf, " . $crlf .
             ...
             " where d.sq_cliente_pai = '".$CL."' and  d.ativo <> 'N' and d.publica = 'S' " . $crlf;
      ...
      if (count($RS) > 0) {
        ...
        ShowHTML('            </ul>');
        ShowHTML('            </div>');
        barra($w_dir . $w_pagina . $par . '&R=' . $w_pagina . $par . '&O=' . $O . '&P1=' . $P1 . '&P2=' . $P2 . '&TP=' . $TP . '&SG=' . $SG . '&CL=' . $CL . MontaFiltro('GET'), ceil(count($RS) / $P4), $P3, $P4, count($RS));
      } else {
  ...
  function Main() {
    ...
    switch ($par) {
      case 'INICIAL' :
        ShowHTML('      <h3>Inicial</h3>');
        Inicial();
        break;
      case 'QUEM' :
        ShowHTML('      <h3>Quem somos</h3>');
        Quem();
        break;
      case 'FALE' :
        ShowHTML('      <h3>Fale conosco</h3>');
        Fale();
        break;
      case 'PROJETO' :
        ShowHTML('      <h3>Projeto</h3>');
        Projeto();
        break;
      case 'NOTICIA' :
        ShowHTML('      <h3>Notícias</h3>');
        Noticia();
        break;
      case 'CALENDARIO' :
        ShowHTML('      <h3>Calendário</h3>');
        Calendario();
        break;
      case 'ARQUIVO' :
        ShowHTML('      <h3>Arquivos</h3>');
        Arquivo();
        break;
      case 'ESCOLA' :
        ShowHTML('      <h3>Escolas</h3>');
        Escola();
        break;
